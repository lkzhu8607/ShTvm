...
#ifndef V1_3AB8707_T_20150815_162521
#define V1_3AB8707_T_20150815_162521

 
///////////////////////////////////////////////////////
//Contents:	The row class type definition of b8707_t
//Name:	b8707_t_rowtype
class b8707_t_rowtype {
 
public:

	std::string		m_strTblName;			// 1		打印机-b8707 
	long		m_lVer;			// 2		参数版本 
	...
	int		m_PrinterStopUseFlag;			// a	4	停用标记,0=使用，1=停用 
	wl::tuint8		m_Major;			// a	5	主版本号 
	wl::tuint8		m_Minor;			// a	6	副版本号 
	int		m_NearEmpty;			// a	7	纸将尽 
	int		m_IsEmpty;			// a	8	纸尽 
	int		m_RES_01;			// a	9	[保留] 
	int		m_RES_02;			// a	10	[保留] 
...
public:
	...
	virtual ~b8707_t_rowtype(){;}
	

	//operator == 
	bool operator == (const b8707_t_rowtype & rhs) const
	{
		return this == &rhs;
	}
...
public:
	...
	std::string & Serialize( std::string & strOut )
	{
		...
		strOut += std::string(buf2);
		strOut += std::string("/");
		buf1 = (const char *)reinterpret_cast<char *>(&reinterpret_cast<char&>(m_PrinterStopUseFlag));
		len1 = sizeof(m_PrinterStopUseFlag);
		if( (int)v.size() < ( len1 * 2 + 4 ) ) v.resize( len1 * 2 + 4 );
		buf2 = (char*)(&(v[0]));
		en( buf1, len1, buf2 );
		strOut += std::string(buf2);
		...
		strOut += std::string(buf2);
		strOut += std::string("/");
		buf1 = (const char *)reinterpret_cast<char *>(&reinterpret_cast<char&>(m_NearEmpty));
		len1 = sizeof(m_NearEmpty);
		if( (int)v.size() < ( len1 * 2 + 4 ) ) v.resize( len1 * 2 + 4 );
		buf2 = (char*)(&(v[0]));
		en( buf1, len1, buf2 );
		strOut += std::string(buf2);
		strOut += std::string("/");
		buf1 = (const char *)reinterpret_cast<char *>(&reinterpret_cast<char&>(m_IsEmpty));
		len1 = sizeof(m_IsEmpty);
		if( (int)v.size() < ( len1 * 2 + 4 ) ) v.resize( len1 * 2 + 4 );
		buf2 = (char*)(&(v[0]));
		en( buf1, len1, buf2 );
		strOut += std::string(buf2);
	...
	b8707_t_rowtype & Unserialize( const char * strIn )
	{
		const char *buf1;
		char *buf2;
		buf1 = strIn;
		...
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
		 /*if( *buf2!='}')*/ m_PrinterStopUseFlag = *(int*)buf2; }else return *this; 
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
		...
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
		 /*if( *buf2!='}')*/ m_NearEmpty = *(int*)buf2; }else return *this; 
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
		 /*if( *buf2!='}')*/ m_IsEmpty = *(int*)buf2; }else return *this; 
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
	...
	b8707_t_rowtype & Unserialize( wl::tchar * p_begin, wl::tchar ** pp_end )
	{
		char *buf1 = p_begin;
		char *buf2;
		buf2 = (char*)buf1 + 4;
...
////////////////////////////////////////////////////////////////
//Function:	GetCol_PrinterStopUseFlag
//Effect:	get ref of col. col is PrinterStopUseFlag
//Return:	int & 
	int & GetCol_PrinterStopUseFlag(void)
	{
		return m_PrinterStopUseFlag;
	}
...
//////////////////////////////////////////////////////////
//Function:	GetCol_NearEmpty
//Effect:	get ref of col. col is NearEmpty
//Return:	int & 
	int & GetCol_NearEmpty(void)
	{
		return m_NearEmpty;
	}

///////////////////////////////////////////////////////
//Function:	GetCol_IsEmpty
//Effect:	get ref of col. col is IsEmpty
//Return:	int & 
	int & GetCol_IsEmpty(void)
	{
		return m_IsEmpty;
	}
...
//Return:	return the column number, int.
	int GetColNumber( std::string strColName ) 
	{
		...
		if( strColName == "PrinterStopUseFlag" )
		{
			return 10;
		}
		if( strColName == "Major" )
...
//Return:	return col value in string format.
	...
	STRINGT GetColStr( int iColNum )
	{
		...
		if( iColNum == 10 )
		{
			return wl::SStrf::sltoa(m_PrinterStopUseFlag);
		}
		if( iColNum == 11 )
		{
		...
		if( iColNum == 13 )
		{
			return wl::SStrf::sltoa(m_NearEmpty);
		}
		if( iColNum == 14 )
		{
			return wl::SStrf::sltoa(m_IsEmpty);
		}
		if( iColNum == 15 )
		{
...
//Return:	no return.
	...
	void SetColVal( int iColNum, STRINGT strValPARA )
	{
		...
		if( iColNum == 10 )
		{
			m_PrinterStopUseFlag=wl::SStrf::satol(strValPARA);
		}
		if( iColNum == 11 )
		{
		...
		if( iColNum == 13 )
		{
			m_NearEmpty=wl::SStrf::satol(strValPARA);
		}
		if( iColNum == 14 )
		{
			m_IsEmpty=wl::SStrf::satol(strValPARA);
		}
		if( iColNum == 15 )
		{
...
private:
	...
	b8707_t_rowtype m_EmptyRow;
	std::vector<b8707_t_rowtype>  m_DATAcorpora;
 
public:

	typedef b8707_t_rowtype 	ROWTYPE;
	typedef std::vector<long> 	RPSTYPE; //RPS(return pointer set) type definition
	typedef std::vector<b8707_t_rowtype>::iterator 	TBLITTYPE;
...
public:
	...
	//operator == 
	bool operator == (const b8707_t & rhs) const
	{
		return this == &rhs;
	}
...
public:
	...
	b8707_t &  Unserialize( const char * strIn , int biAppend = 0 ) //if append==0, it would over-write the current tbl. 
	{
		char *p1,*p2;
		p1 = (char*)strIn;
		if(biAppend==0) Clear();
	...
	b8707_t & Unserialize( wl::SCake & ckIn , int biAppend = 0 )
	{
		char *p1,*p2;
		p1 = p2 = (char*)ckIn.buf();
		if(biAppend==0) Clear();
...
//Return:	b8707_t_rowtype&
	b8707_t_rowtype & GetRow(long lRowNum)
	{
		if(lRowNum>=0&&lRowNum<(long)m_DATAcorpora.size())
		return m_DATAcorpora[lRowNum];
		else 
		{
			b8707_t_rowtype tmpEmptyrow;
...
//Return:	b8707_t_rowtype&
	b8707_t_rowtype & GetRow(const std::vector<long> & vRps, long lRowNum)
	{
		long ltmp;
		do{
		{
...
//Return:	no return
	void Add(const b8707_t_rowtype & varRow)
	{
...
//Return:	b8707_t_rowtype&
	b8707_t_rowtype & SelE1_strTblName(std::string strVal) 
	{
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8707_t_rowtype&
	b8707_t_rowtype & SelE1_lVer(long iVal) 
	{
		std::vector<long> vRps ;
		SelE_lVer( iVal, vRps );
		return GetRow( vRps, 0 );
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8707_t_rowtype&
	b8707_t_rowtype & SelE1_strAffectDateTime(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8707_t_rowtype&
	b8707_t_rowtype & SelE1_biDelFlag(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8707_t_rowtype&
	b8707_t_rowtype & SelE1_biIsAffect(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8707_t_rowtype&
	b8707_t_rowtype & SelE1_strStampDateTime(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8707_t_rowtype&
	b8707_t_rowtype & SelE1_strDataRef(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8707_t_rowtype&
	b8707_t_rowtype & SelE1_ConnState(int iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8707_t_rowtype&
	b8707_t_rowtype & SelE1_RtnVal(int iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8707_t_rowtype&
	b8707_t_rowtype & SelE1_BigErr(int iVal) 
	{
		std::vector<long> vRps ;
...
/////////////////////////////////////////////////////////////////
//Function:	SelE_PrinterStopUseFlag
//Effect:	select and build RPS where col=val use traverse
//Return:	no return
	void SelE_PrinterStopUseFlag(int iVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //(ni)
	{
		for(long ltmp=0;ltmp<(long)m_DATAcorpora.size();ltmp++)
		if(m_DATAcorpora[ltmp].m_PrinterStopUseFlag==iVal)
		if( !pRefRps || !pRefRps->empty() && std::binary_search( pRefRps->begin(), pRefRps->end(), ltmp ) ) vRps.push_back( ltmp );
	}

//////////////////////////////////////////////////////////
//Function:	SelE1_PrinterStopUseFlag
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8707_t_rowtype&
	b8707_t_rowtype & SelE1_PrinterStopUseFlag(int iVal) 
	{
		std::vector<long> vRps ;
		SelE_PrinterStopUseFlag( iVal, vRps );
		return GetRow( vRps, 0 );
	}

//////////////////////////////////////////////////////////
//Function:	SelEc_PrinterStopUseFlag
//Effect:	select and build RPS where col=val. clear rps first.
//Return:	no return
	void SelEc_PrinterStopUseFlag(int iVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //()
	{
		vRps.clear();
		SelE_PrinterStopUseFlag(iVal, vRps, pRefRps);
	}
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8707_t_rowtype&
	b8707_t_rowtype & SelE1_Major(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8707_t_rowtype&
	b8707_t_rowtype & SelE1_Minor(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
///////////////////////////////////////////////////////////////
//Function:	SelE_NearEmpty
//Effect:	select and build RPS where col=val use traverse
//Return:	no return
	void SelE_NearEmpty(int iVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //(ni)
	{
		for(long ltmp=0;ltmp<(long)m_DATAcorpora.size();ltmp++)
		if(m_DATAcorpora[ltmp].m_NearEmpty==iVal)
		if( !pRefRps || !pRefRps->empty() && std::binary_search( pRefRps->begin(), pRefRps->end(), ltmp ) ) vRps.push_back( ltmp );
	}

////////////////////////////////////////////////////////////
//Function:	SelE1_NearEmpty
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8707_t_rowtype&
	b8707_t_rowtype & SelE1_NearEmpty(int iVal) 
	{
		std::vector<long> vRps ;
		SelE_NearEmpty( iVal, vRps );
		return GetRow( vRps, 0 );
	}

////////////////////////////////////////////////////////////
//Function:	SelEc_NearEmpty
//Effect:	select and build RPS where col=val. clear rps first.
//Return:	no return
	void SelEc_NearEmpty(int iVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //()
	{
		vRps.clear();
		SelE_NearEmpty(iVal, vRps, pRefRps);
	}

/////////////////////////////////////////////////////////////////
//Function:	SelE_IsEmpty
//Effect:	select and build RPS where col=val use traverse
//Return:	no return
	void SelE_IsEmpty(int iVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //(ni)
	{
		for(long ltmp=0;ltmp<(long)m_DATAcorpora.size();ltmp++)
		if(m_DATAcorpora[ltmp].m_IsEmpty==iVal)
		if( !pRefRps || !pRefRps->empty() && std::binary_search( pRefRps->begin(), pRefRps->end(), ltmp ) ) vRps.push_back( ltmp );
	}

///////////////////////////////////////////////////////////////
//Function:	SelE1_IsEmpty
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8707_t_rowtype&
	b8707_t_rowtype & SelE1_IsEmpty(int iVal) 
	{
		std::vector<long> vRps ;
		SelE_IsEmpty( iVal, vRps );
		return GetRow( vRps, 0 );
	}

////////////////////////////////////////////////////////
//Function:	SelEc_IsEmpty
//Effect:	select and build RPS where col=val. clear rps first.
//Return:	no return
	void SelEc_IsEmpty(int iVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //()
	{
		vRps.clear();
		SelE_IsEmpty(iVal, vRps, pRefRps);
	}
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8707_t_rowtype&
	b8707_t_rowtype & SelE1_RES_01(int iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8707_t_rowtype&
	b8707_t_rowtype & SelE1_RES_02(int iVal) 
	{
		std::vector<long> vRps ;
