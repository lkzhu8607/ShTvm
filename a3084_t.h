...
public:
	...
	std::string		m_strTblName;			// 1		7.0.22  3084-自动售票机车站界面定义报文 
	long		m_lVer;			// 2		参数版本 
	...
	long		m_LineCode;			// a	1	线路代码 线路代码，BCD码 Byte 1 
	std::string		m_strPicFn;			// a	2	文件名 单条线路示意图的文件名 String 24 
	long		m_StaNum;			// a	3	车站记录数 16进制表示，例50个车站表示为0x32 Byte 1 
	u8arr_t<4>		m_RES_01;			// a	4	保留  Byte 4 
	uiarr_t<long,64>		m_StaCode;			// a	5	车站标识码 车站编号，BCD码 Byte 1 
	uiarr_t<long,64>		m_StaButtX;			// a	6	x坐标 车站按钮在车站选择界面上的x坐标，单位：像素 Word 2 
	uiarr_t<long,64>		m_StaButtY;			// a	7	y坐标 车站按钮在车站选择界面上的y坐标，单位：像素 Word 2 
	uiarr_t<long,64>		m_StaButtWidth;			// a	8	长度 按钮区域的横向长度，单位：像素 Byte 1 
	uiarr_t<long,64>		m_StaButtHeight;			// a	9	宽度 按钮区域的纵向宽度，单位：像素 Byte 1 
	uiarr_t<tuint8,64>		m_RES_02;			// a	10	保留  Byte 1 
