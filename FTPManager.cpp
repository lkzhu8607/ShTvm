...
long CFTPManager::getLocalFileSize(const char *filename)  
{
	struct stat buf;  
...
{  
    ...
    trace("@@@@Response: %s\n", serverResponse(m_cmdSocket).c_str());  
 	...
 	if(NULL==strrchr(strLocalFile.c_str(),  '/'))
	{
...
int update_software_by_ftp()
{
	char configure_filename[128]="../conf/FTPManager.conf";
	char value[128]={0,};
	char cmd_str[512]={0,};
	
	ini_read_string(configure_filename,"FTPServerIP",value,sizeof(value)-1);
	std::string FTPServerIP(value);
	
	ini_read_string(configure_filename,"UserName",value,sizeof(value)-1);
	std::string UserName(value);
	
	ini_read_string(configure_filename,"PassWord",value,sizeof(value)-1);
	std::string PassWord(value);

	ini_read_string(configure_filename,"ServerFilePath",value,sizeof(value)-1);
	std::string ServerFilePath(value);
	
	ini_read_string(configure_filename,"ClientFilePath",value,sizeof(value)-1);
	std::string ClientFilePath(value);
	
	ini_read_string(configure_filename,"UpdatePackageName",value,sizeof(value)-1);
	std::string UpdatePackageName(value);
	...
	if(FtpClient.inputUserName(UserName)==-1)
	{
		return -1;
	}
	if(-1==FtpClient.inputPassWord(PassWord))
	{
		return -1;
	}
		
	long remote_filesize=FtpClient.getFileLength(RemoteProgramName);
	ret=FtpClient.Get(RemoteProgramName,LocaltmpProgramName);
	
	long local_filesize=FtpClient.getLocalFileSize(LocaltmpProgramName.c_str());
	if(ret==0 &&remote_filesize==local_filesize)
	{
		printf("download file  success......\n");
		
		snprintf(cmd_str,sizeof(cmd_str)-1,"mv %s %s;",LocaltmpProgramName.c_str(),LocalProgramName.c_str());
		printf("%s\n",cmd_str);
		system(cmd_str);
