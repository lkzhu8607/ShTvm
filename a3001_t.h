...
#ifndef V1_3AA3001_T_20140523_155934
#define V1_3AA3001_T_20140523_155934

 
///////////////////////////////////////////////////////
//Contents:	The row class type definition of a3001_t
//Name:	a3001_t_rowtype
class a3001_t_rowtype {
 
public:

	std::string		m_strTblName;			// 1		a3001 
	long		m_lVer;			// 2		参数版本 
	...
	u8arr_t<4>		m_BeginNode;			// a	1	车站标识码 SC节点标识码 Block 4 
	u8arr_t<4>		m_RunNode;			// a	2	车站标识码 SC节点标识码 Block 4 
	u8arr_t<2>		m_ModeCode;			// a	3	系统运行模式代码  Word 2 
	long		m_lOperatorNum;			// a	4	操作员编号 发起者 Long 4 
	wl::tuint8		m_RES01;			// a	5	[保留] 
	u8arr_t<2>		m_RES02;			// a	6	[保留] 
	wl::tuint8		m_RES03;			// a	7	[保留] 
...
public:
	...
	virtual ~a3001_t_rowtype(){;}
	

	//operator == 
	bool operator == (const a3001_t_rowtype & rhs) const
	{
		return this == &rhs;
	}
...
public:
	...
	std::string & Serialize( std::string & strOut )
	{
		...
		strOut += std::string(buf2);
		strOut += std::string("/");
		buf1 = (const char *)reinterpret_cast<char *>(&reinterpret_cast<char&>(m_ModeCode));
		len1 = sizeof(m_ModeCode);
		if( (int)v.size() < ( len1 * 2 + 4 ) ) v.resize( len1 * 2 + 4 );
		buf2 = (char*)(&(v[0]));
		en( buf1, len1, buf2 );
		strOut += std::string(buf2);
	...
	a3001_t_rowtype & Unserialize( const char * strIn )
	{
		const char *buf1;
		char *buf2;
		buf1 = strIn;
		...
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
		 /*if( *buf2!='}')*/ m_ModeCode = *(u8arr_t<2>*)buf2; }else return *this; 
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
	...
	a3001_t_rowtype & Unserialize( wl::tchar * p_begin, wl::tchar ** pp_end )
	{
		char *buf1 = p_begin;
		char *buf2;
		buf2 = (char*)buf1 + 4;
...
////////////////////////////////////////////////////////
//Function:	GetCol_ModeCode
//Effect:	get ref of col. col is ModeCode
//Return:	u8arr_t<2> & 
	u8arr_t<2> & GetCol_ModeCode(void)
	{
		return m_ModeCode;
	}
...
//Return:	return col value in string format.
	...
	STRINGT GetColStr( int iColNum )
	{
		...
		if( iColNum == 9 )
		{
			return wl::SStrf::b2s(m_ModeCode);
		}
		if( iColNum == 10 )
		{
...
//Return:	no return.
	...
	void SetColVal( int iColNum, STRINGT strValPARA )
	{
		...
		if( iColNum == 9 )
		{
			wl::SStrf::s2b(strValPARA,m_ModeCode);
		}
		if( iColNum == 10 )
		{
...
private:
	...
	a3001_t_rowtype m_EmptyRow;
	std::vector<a3001_t_rowtype>  m_DATAcorpora;
 
public:

	typedef a3001_t_rowtype 	ROWTYPE;
	typedef std::vector<long> 	RPSTYPE; //RPS(return pointer set) type definition
	typedef std::vector<a3001_t_rowtype>::iterator 	TBLITTYPE;
...
public:
	...
	//operator == 
	bool operator == (const a3001_t & rhs) const
	{
		return this == &rhs;
	}
...
public:
	...
	a3001_t &  Unserialize( const char * strIn , int biAppend = 0 ) //if append==0, it would over-write the current tbl. 
	{
		char *p1,*p2;
		p1 = (char*)strIn;
		if(biAppend==0) Clear();
	...
	a3001_t & Unserialize( wl::SCake & ckIn , int biAppend = 0 )
	{
		char *p1,*p2;
		p1 = p2 = (char*)ckIn.buf();
		if(biAppend==0) Clear();
...
//Return:	a3001_t_rowtype&
	a3001_t_rowtype & GetRow(long lRowNum)
	{
		if(lRowNum>=0&&lRowNum<(long)m_DATAcorpora.size())
		return m_DATAcorpora[lRowNum];
		else 
		{
			a3001_t_rowtype tmpEmptyrow;
...
//Return:	a3001_t_rowtype&
	a3001_t_rowtype & GetRow(const std::vector<long> & vRps, long lRowNum)
	{
		long ltmp;
		do{
		{
...
//Return:	no return
	void Add(const a3001_t_rowtype & varRow)
	{
...
//Return:	a3001_t_rowtype&
	a3001_t_rowtype & SelE1_strTblName(std::string strVal) 
	{
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3001_t_rowtype&
	a3001_t_rowtype & SelE1_lVer(long iVal) 
	{
		std::vector<long> vRps ;
		SelE_lVer( iVal, vRps );
		return GetRow( vRps, 0 );
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3001_t_rowtype&
	a3001_t_rowtype & SelE1_strAffectDateTime(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3001_t_rowtype&
	a3001_t_rowtype & SelE1_biDelFlag(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3001_t_rowtype&
	a3001_t_rowtype & SelE1_biIsAffect(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3001_t_rowtype&
	a3001_t_rowtype & SelE1_strStampDateTime(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3001_t_rowtype&
	a3001_t_rowtype & SelE1_strDataRef(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3001_t_rowtype&
	a3001_t_rowtype & SelE1_BeginNode(u8arr_t<4> aVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3001_t_rowtype&
	a3001_t_rowtype & SelE1_RunNode(u8arr_t<4> aVal) 
	{
		std::vector<long> vRps ;
...
///////////////////////////////////////////////////////
//Function:	SelE_ModeCode
//Effect:	select and build RPS where col=val use traverse
//Return:	no return
	void SelE_ModeCode(u8arr_t<2> aVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //(ni)
	{
		for(long ltmp=0;ltmp<(long)m_DATAcorpora.size();ltmp++)
		if(m_DATAcorpora[ltmp].m_ModeCode==aVal)
		if( !pRefRps || !pRefRps->empty() && std::binary_search( pRefRps->begin(), pRefRps->end(), ltmp ) ) vRps.push_back( ltmp );
	}

///////////////////////////////////////////////////////////
//Function:	SelE1_ModeCode
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3001_t_rowtype&
	a3001_t_rowtype & SelE1_ModeCode(u8arr_t<2> aVal) 
	{
		std::vector<long> vRps ;
		SelE_ModeCode( aVal, vRps );
		return GetRow( vRps, 0 );
	}

////////////////////////////////////////////////////////
//Function:	SelEc_ModeCode
//Effect:	select and build RPS where col=val. clear rps first.
//Return:	no return
	void SelEc_ModeCode(u8arr_t<2> aVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //()
	{
		vRps.clear();
		SelE_ModeCode(aVal, vRps, pRefRps);
	}
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3001_t_rowtype&
	a3001_t_rowtype & SelE1_lOperatorNum(long iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3001_t_rowtype&
	a3001_t_rowtype & SelE1_RES01(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3001_t_rowtype&
	a3001_t_rowtype & SelE1_RES02(u8arr_t<2> aVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3001_t_rowtype&
	a3001_t_rowtype & SelE1_RES03(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
