...
#ifndef V1_3AA3007_T_20140523_161042
#define V1_3AA3007_T_20140523_161042

 
///////////////////////////////////////////////////////
//Contents:	The row class type definition of a3007_t
//Name:	a3007_t_rowtype
class a3007_t_rowtype {
 
public:

	std::string		m_strTblName;			// 1		7.0.10  3007-线路名称表。此类交互应用于传送线路名称信息。它最初由清分系统主 
	long		m_lVer;			// 2		参数版本 
	...
	u8arr_t<4>		m_CcNode;			// a	1	CC标识码 CC的节点标识码 Block 4 
	std::string		m_strLineNameEn;			// a	2	线路名称（英文）  String 32 
	long		m_lLineNameCh;			// a	3	线路名称（中文） 清分系统本地语言资源文件指针 Pointer 4 
	u8arr_t<4>		m_RES_01;			// a	4	[保留]  Byte 4 
	wl::tuint8		m_RES_02;			// a	5	[保留]  Byte 1 
...
public:
	...
	virtual ~a3007_t_rowtype(){;}
	

	//operator == 
	bool operator == (const a3007_t_rowtype & rhs) const
	{
		return this == &rhs;
	}
...
public:
	...
	std::string & Serialize( std::string & strOut )
	{
		...
		strOut += std::string(buf2);
		strOut += std::string("/");
		buf1 = (const char *)m_strLineNameEn.c_str();
		len1 = (long)(m_strLineNameEn.length()*sizeof(char)+sizeof(char));
		if( (int)v.size() < ( len1 * 2 + 4 ) ) v.resize( len1 * 2 + 4 );
		buf2 = (char*)(&(v[0]));
		en( buf1, len1, buf2 );
		strOut += std::string(buf2);
		strOut += std::string("/");
		buf1 = (const char *)reinterpret_cast<char *>(&reinterpret_cast<char&>(m_lLineNameCh));
		len1 = sizeof(m_lLineNameCh);
		if( (int)v.size() < ( len1 * 2 + 4 ) ) v.resize( len1 * 2 + 4 );
		buf2 = (char*)(&(v[0]));
		en( buf1, len1, buf2 );
		strOut += std::string(buf2);
	...
	a3007_t_rowtype & Unserialize( const char * strIn )
	{
		const char *buf1;
		char *buf2;
		buf1 = strIn;
		...
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
		 /*if( *buf2!='}')*/ m_strLineNameEn = (char*)buf2; }else return *this; 
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
		 /*if( *buf2!='}')*/ m_lLineNameCh = *(long*)buf2; }else return *this; 
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
	...
	a3007_t_rowtype & Unserialize( wl::tchar * p_begin, wl::tchar ** pp_end )
	{
		char *buf1 = p_begin;
		char *buf2;
		buf2 = (char*)buf1 + 4;
		...
		buf2 = (char*)buf1 + 4;
		buf1 = buf2 + *(wl::tuint32*)buf1;
		m_strLineNameEn = (char*)buf2;
		buf2 = (char*)buf1 + 4;
		buf1 = buf2 + *(wl::tuint32*)buf1;
		m_lLineNameCh = *(long*)buf2;
		buf2 = (char*)buf1 + 4;
		buf1 = buf2 + *(wl::tuint32*)buf1;
...
///////////////////////////////////////////////////////////////
//Function:	GetCol_strLineNameEn
//Effect:	get ref of col. col is strLineNameEn
//Return:	std::string & 
	std::string & GetCol_strLineNameEn(void)
	{
		return m_strLineNameEn;
	}

////////////////////////////////////////////////////////
//Function:	GetCol_lLineNameCh
//Effect:	get ref of col. col is lLineNameCh
//Return:	long & 
	long & GetCol_lLineNameCh(void)
	{
		return m_lLineNameCh;
	}
...
//Return:	return the column number, int.
	int GetColNumber( std::string strColName ) 
	{
		...
		if( strColName == "strLineNameEn" )
		{
			return 8;
		}
		if( strColName == "lLineNameCh" )
		{
			return 9;
		}
...
//Return:	return col value in string format.
	...
	STRINGT GetColStr( int iColNum )
	{
		...
		if( iColNum == 9 )
		{
			return wl::SStrf::sltoa(m_lLineNameCh);
		}
		if( iColNum == 10 )
		{
...
//Return:	no return.
	...
	void SetColVal( int iColNum, STRINGT strValPARA )
	{
		...
		if( iColNum == 8 )
		{
			m_strLineNameEn=strValPARA;
		}
		if( iColNum == 9 )
		{
			m_lLineNameCh=wl::SStrf::satol(strValPARA);
		}
		if( iColNum == 10 )
		{
...
private:
	...
	a3007_t_rowtype m_EmptyRow;
	std::vector<a3007_t_rowtype>  m_DATAcorpora;
 
public:

	typedef a3007_t_rowtype 	ROWTYPE;
	typedef std::vector<long> 	RPSTYPE; //RPS(return pointer set) type definition
	typedef std::vector<a3007_t_rowtype>::iterator 	TBLITTYPE;
...
public:
	...
	//operator == 
	bool operator == (const a3007_t & rhs) const
	{
		return this == &rhs;
	}
...
public:
	...
	a3007_t &  Unserialize( const char * strIn , int biAppend = 0 ) //if append==0, it would over-write the current tbl. 
	{
		char *p1,*p2;
		p1 = (char*)strIn;
		if(biAppend==0) Clear();
	...
	a3007_t & Unserialize( wl::SCake & ckIn , int biAppend = 0 )
	{
		char *p1,*p2;
		p1 = p2 = (char*)ckIn.buf();
		if(biAppend==0) Clear();
...
//Return:	a3007_t_rowtype&
	a3007_t_rowtype & GetRow(long lRowNum)
	{
		if(lRowNum>=0&&lRowNum<(long)m_DATAcorpora.size())
		return m_DATAcorpora[lRowNum];
		else 
		{
			a3007_t_rowtype tmpEmptyrow;
...
//Return:	a3007_t_rowtype&
	a3007_t_rowtype & GetRow(const std::vector<long> & vRps, long lRowNum)
	{
		long ltmp;
		do{
		{
...
//Return:	no return
	void Add(const a3007_t_rowtype & varRow)
	{
...
//Return:	a3007_t_rowtype&
	a3007_t_rowtype & SelE1_strTblName(std::string strVal) 
	{
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3007_t_rowtype&
	a3007_t_rowtype & SelE1_lVer(long iVal) 
	{
		std::vector<long> vRps ;
		SelE_lVer( iVal, vRps );
		return GetRow( vRps, 0 );
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3007_t_rowtype&
	a3007_t_rowtype & SelE1_strAffectDateTime(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3007_t_rowtype&
	a3007_t_rowtype & SelE1_biDelFlag(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3007_t_rowtype&
	a3007_t_rowtype & SelE1_biIsAffect(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3007_t_rowtype&
	a3007_t_rowtype & SelE1_strStampDateTime(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3007_t_rowtype&
	a3007_t_rowtype & SelE1_strDataRef(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3007_t_rowtype&
	a3007_t_rowtype & SelE1_CcNode(u8arr_t<4> aVal) 
	{
		std::vector<long> vRps ;
...
/////////////////////////////////////////////////////////////
//Function:	SelE_strLineNameEn
//Effect:	select and build RPS where col=val use traverse
//Return:	no return
	void SelE_strLineNameEn(std::string strVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //(ni)
	{
		for(long ltmp=0;ltmp<(long)m_DATAcorpora.size();ltmp++)
		if(m_DATAcorpora[ltmp].m_strLineNameEn==strVal)
		if( !pRefRps || !pRefRps->empty() && std::binary_search( pRefRps->begin(), pRefRps->end(), ltmp ) ) vRps.push_back( ltmp );
	}

///////////////////////////////////////////////////////////
//Function:	SelE1_strLineNameEn
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3007_t_rowtype&
	a3007_t_rowtype & SelE1_strLineNameEn(std::string strVal) 
	{
		std::vector<long> vRps ;
		SelE_strLineNameEn( strVal, vRps );
		return GetRow( vRps, 0 );
	}

/////////////////////////////////////////////////////////
//Function:	SelEc_strLineNameEn
//Effect:	select and build RPS where col=val. clear rps first.
//Return:	no return
	void SelEc_strLineNameEn(std::string strVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //()
	{
		vRps.clear();
		SelE_strLineNameEn(strVal, vRps, pRefRps);
	}

//////////////////////////////////////////////////////////
//Function:	SelE_lLineNameCh
//Effect:	select and build RPS where col=val use traverse
//Return:	no return
	void SelE_lLineNameCh(long iVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //(ni)
	{
		for(long ltmp=0;ltmp<(long)m_DATAcorpora.size();ltmp++)
		if(m_DATAcorpora[ltmp].m_lLineNameCh==iVal)
		if( !pRefRps || !pRefRps->empty() && std::binary_search( pRefRps->begin(), pRefRps->end(), ltmp ) ) vRps.push_back( ltmp );
	}

////////////////////////////////////////////////////////////////
//Function:	SelE1_lLineNameCh
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3007_t_rowtype&
	a3007_t_rowtype & SelE1_lLineNameCh(long iVal) 
	{
		std::vector<long> vRps ;
		SelE_lLineNameCh( iVal, vRps );
		return GetRow( vRps, 0 );
	}

///////////////////////////////////////////////////////
//Function:	SelEc_lLineNameCh
//Effect:	select and build RPS where col=val. clear rps first.
//Return:	no return
	void SelEc_lLineNameCh(long iVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //()
	{
		vRps.clear();
		SelE_lLineNameCh(iVal, vRps, pRefRps);
	}
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3007_t_rowtype&
	a3007_t_rowtype & SelE1_RES_01(u8arr_t<4> aVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3007_t_rowtype&
	a3007_t_rowtype & SelE1_RES_02(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
