...
#ifndef V1_3AB8601_T_20150303_164932
#define V1_3AB8601_T_20150303_164932

 
///////////////////////////////////////////////////////
//Contents:	The row class type definition of b8601_t
//Name:	b8601_t_rowtype
class b8601_t_rowtype {
 
public:

	std::string		m_strTblName;			// 1		moneylog-b8601	为了删除快，不要用索引 
	long		m_lVer;			// 2		参数版本 
	...
	std::string		m_LogStr;			// a	1	内容 
	int		m_Type;			// a	2	类型。0=front,1=back,2=sheet 
	int		m_RES_1;			// a	3	[保留] 
...
public:
	...
	virtual ~b8601_t_rowtype(){;}
	

	//operator == 
	bool operator == (const b8601_t_rowtype & rhs) const
	{
		return this == &rhs;
	}
...
public:
	...
	std::string & Serialize( std::string & strOut )
	{
		...
		strOut += std::string(buf2);
		strOut += std::string("/");
		buf1 = (const char *)m_LogStr.c_str();
		len1 = (long)(m_LogStr.length()*sizeof(char)+sizeof(char));
		if( (int)v.size() < ( len1 * 2 + 4 ) ) v.resize( len1 * 2 + 4 );
		buf2 = (char*)(&(v[0]));
		en( buf1, len1, buf2 );
		strOut += std::string(buf2);
		strOut += std::string("/");
		buf1 = (const char *)reinterpret_cast<char *>(&reinterpret_cast<char&>(m_Type));
		len1 = sizeof(m_Type);
		if( (int)v.size() < ( len1 * 2 + 4 ) ) v.resize( len1 * 2 + 4 );
		buf2 = (char*)(&(v[0]));
		en( buf1, len1, buf2 );
		strOut += std::string(buf2);
		strOut += std::string("/");
		buf1 = (const char *)reinterpret_cast<char *>(&reinterpret_cast<char&>(m_RES_1));
		len1 = sizeof(m_RES_1);
		if( (int)v.size() < ( len1 * 2 + 4 ) ) v.resize( len1 * 2 + 4 );
		buf2 = (char*)(&(v[0]));
		en( buf1, len1, buf2 );
	...
	b8601_t_rowtype & Unserialize( const char * strIn )
	{
		const char *buf1;
		char *buf2;
		buf1 = strIn;
		...
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
		 /*if( *buf2!='}')*/ m_LogStr = (char*)buf2; }else return *this; 
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
		 /*if( *buf2!='}')*/ m_Type = *(int*)buf2; }else return *this; 
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
		 /*if( *buf2!='}')*/ m_RES_1 = *(int*)buf2; }else return *this; 
		return *this;
	}
	...
	b8601_t_rowtype & Unserialize( wl::tchar * p_begin, wl::tchar ** pp_end )
	{
		char *buf1 = p_begin;
		char *buf2;
		buf2 = (char*)buf1 + 4;
...
////////////////////////////////////////////////////////////////
//Function:	GetCol_LogStr
//Effect:	get ref of col. col is LogStr
//Return:	std::string & 
	std::string & GetCol_LogStr(void)
	{
		return m_LogStr;
	}
...
//Return:	int & 
	int & GetCol_Type(void)
	{
		return m_Type;
	}

////////////////////////////////////////////////////////
//Function:	GetCol_RES_1
//Effect:	get ref of col. col is RES_1
//Return:	int & 
	int & GetCol_RES_1(void)
	{
		return m_RES_1;
	}
...
//Return:	return col value in string format.
	...
	STRINGT GetColStr( int iColNum )
	{
		...
		if( iColNum == 8 )
		{
			return wl::SStrf::sltoa(m_Type);
		}
		if( iColNum == 9 )
		{
			return wl::SStrf::sltoa(m_RES_1);
		}
...
//Return:	no return.
	...
	void SetColVal( int iColNum, STRINGT strValPARA )
	{
		...
		if( iColNum == 8 )
		{
			m_Type=wl::SStrf::satol(strValPARA);
		}
		if( iColNum == 9 )
		{
			m_RES_1=wl::SStrf::satol(strValPARA);
		}
	}
...
private:
	...
	b8601_t_rowtype m_EmptyRow;
	std::vector<b8601_t_rowtype>  m_DATAcorpora;
 
public:

	typedef b8601_t_rowtype 	ROWTYPE;
	typedef std::vector<long> 	RPSTYPE; //RPS(return pointer set) type definition
	typedef std::vector<b8601_t_rowtype>::iterator 	TBLITTYPE;
...
public:
	...
	//operator == 
	bool operator == (const b8601_t & rhs) const
	{
		return this == &rhs;
	}
...
public:
	...
	b8601_t &  Unserialize( const char * strIn , int biAppend = 0 ) //if append==0, it would over-write the current tbl. 
	{
		char *p1,*p2;
		p1 = (char*)strIn;
		if(biAppend==0) Clear();
	...
	b8601_t & Unserialize( wl::SCake & ckIn , int biAppend = 0 )
	{
		char *p1,*p2;
		p1 = p2 = (char*)ckIn.buf();
		if(biAppend==0) Clear();
...
//Return:	b8601_t_rowtype&
	b8601_t_rowtype & GetRow(long lRowNum)
	{
		if(lRowNum>=0&&lRowNum<(long)m_DATAcorpora.size())
		return m_DATAcorpora[lRowNum];
		else 
		{
			b8601_t_rowtype tmpEmptyrow;
...
//Return:	b8601_t_rowtype&
	b8601_t_rowtype & GetRow(const std::vector<long> & vRps, long lRowNum)
	{
		long ltmp;
		do{
		{
...
//Return:	no return
	void Add(const b8601_t_rowtype & varRow)
	{
...
//Return:	b8601_t_rowtype&
	b8601_t_rowtype & SelE1_strTblName(std::string strVal) 
	{
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8601_t_rowtype&
	b8601_t_rowtype & SelE1_lVer(long iVal) 
	{
		std::vector<long> vRps ;
		SelE_lVer( iVal, vRps );
		return GetRow( vRps, 0 );
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8601_t_rowtype&
	b8601_t_rowtype & SelE1_strAffectDateTime(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8601_t_rowtype&
	b8601_t_rowtype & SelE1_biDelFlag(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8601_t_rowtype&
	b8601_t_rowtype & SelE1_biIsAffect(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8601_t_rowtype&
	b8601_t_rowtype & SelE1_strStampDateTime(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8601_t_rowtype&
	b8601_t_rowtype & SelE1_strDataRef(std::string strVal) 
	{
		std::vector<long> vRps ;
...
/////////////////////////////////////////////////////////
//Function:	SelE_LogStr
//Effect:	select and build RPS where col=val use traverse
//Return:	no return
	void SelE_LogStr(std::string strVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //(ni)
	{
		for(long ltmp=0;ltmp<(long)m_DATAcorpora.size();ltmp++)
		if(m_DATAcorpora[ltmp].m_LogStr==strVal)
		if( !pRefRps || !pRefRps->empty() && std::binary_search( pRefRps->begin(), pRefRps->end(), ltmp ) ) vRps.push_back( ltmp );
	}

///////////////////////////////////////////////////////////////
//Function:	SelE1_LogStr
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8601_t_rowtype&
	b8601_t_rowtype & SelE1_LogStr(std::string strVal) 
	{
		std::vector<long> vRps ;
		SelE_LogStr( strVal, vRps );
		return GetRow( vRps, 0 );
	}

////////////////////////////////////////////////////////////////
//Function:	SelEc_LogStr
//Effect:	select and build RPS where col=val. clear rps first.
//Return:	no return
	void SelEc_LogStr(std::string strVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //()
	{
		vRps.clear();
		SelE_LogStr(strVal, vRps, pRefRps);
	}
...
//Effect:	select and build RPS where col=val use traverse
//Return:	no return
	void SelE_Type(int iVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //(ni)
	{
		for(long ltmp=0;ltmp<(long)m_DATAcorpora.size();ltmp++)
		if(m_DATAcorpora[ltmp].m_Type==iVal)
		if( !pRefRps || !pRefRps->empty() && std::binary_search( pRefRps->begin(), pRefRps->end(), ltmp ) ) vRps.push_back( ltmp );
	}
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8601_t_rowtype&
	b8601_t_rowtype & SelE1_Type(int iVal) 
	{
		std::vector<long> vRps ;
		SelE_Type( iVal, vRps );
		return GetRow( vRps, 0 );
...
//Effect:	select and build RPS where col=val. clear rps first.
//Return:	no return
	void SelEc_Type(int iVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //()
	{
		vRps.clear();
		SelE_Type(iVal, vRps, pRefRps);
	}
...
//Effect:	select and build RPS where col=val use traverse
//Return:	no return
	void SelE_RES_1(int iVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //(ni)
	{
		for(long ltmp=0;ltmp<(long)m_DATAcorpora.size();ltmp++)
		if(m_DATAcorpora[ltmp].m_RES_1==iVal)
		if( !pRefRps || !pRefRps->empty() && std::binary_search( pRefRps->begin(), pRefRps->end(), ltmp ) ) vRps.push_back( ltmp );
	}

/////////////////////////////////////////////////////////
//Function:	SelE1_RES_1
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8601_t_rowtype&
	b8601_t_rowtype & SelE1_RES_1(int iVal) 
	{
		std::vector<long> vRps ;
		SelE_RES_1( iVal, vRps );
		return GetRow( vRps, 0 );
	}

//////////////////////////////////////////////////////////
//Function:	SelEc_RES_1
//Effect:	select and build RPS where col=val. clear rps first.
//Return:	no return
	void SelEc_RES_1(int iVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //()
	{
		vRps.clear();
		SelE_RES_1(iVal, vRps, pRefRps);
	}
