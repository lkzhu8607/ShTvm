...
#ifndef V1_3AA3006_T_20140523_160820
#define V1_3AA3006_T_20140523_160820

 
///////////////////////////////////////////////////////
//Contents:	The row class type definition of a3006_t
//Name:	a3006_t_rowtype
class a3006_t_rowtype {
 
public:

	std::string		m_strTblName;			// 1		7.0.9  3006-车站名称/线路设置表。此类交互应用于传送车站名称/线路设置信息。它 
	long		m_lVer;			// 2		参数版本 
	...
	u8arr_t<4>		m_StaNode;			// a	1	车站标识码 SC节点标识码 Block 4 
	std::string		m_strStaNameEn;			// a	2	车站名称（英文）  String 32 
	long		m_lStaNameCh;			// a	3	车站名称（中文） 清分系统本地语言资源文件指针 Pointer 4 
...
public:
	...
	virtual ~a3006_t_rowtype(){;}
	

	//operator == 
	bool operator == (const a3006_t_rowtype & rhs) const
	{
		return this == &rhs;
	}
...
public:
	...
	std::string & Serialize( std::string & strOut )
	{
		...
		strOut += std::string(buf2);
		strOut += std::string("/");
		buf1 = (const char *)reinterpret_cast<char *>(&reinterpret_cast<char&>(m_StaNode));
		len1 = sizeof(m_StaNode);
		if( (int)v.size() < ( len1 * 2 + 4 ) ) v.resize( len1 * 2 + 4 );
		buf2 = (char*)(&(v[0]));
		en( buf1, len1, buf2 );
		strOut += std::string(buf2);
	...
	a3006_t_rowtype & Unserialize( const char * strIn )
	{
		const char *buf1;
		char *buf2;
		buf1 = strIn;
		...
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
		 /*if( *buf2!='}')*/ m_StaNode = *(u8arr_t<4>*)buf2; }else return *this; 
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
	...
	a3006_t_rowtype & Unserialize( wl::tchar * p_begin, wl::tchar ** pp_end )
	{
		char *buf1 = p_begin;
		char *buf2;
		buf2 = (char*)buf1 + 4;
...
////////////////////////////////////////////////////////////////
//Function:	GetCol_StaNode
//Effect:	get ref of col. col is StaNode
//Return:	u8arr_t<4> & 
	u8arr_t<4> & GetCol_StaNode(void)
	{
		return m_StaNode;
	}
...
//Return:	no return.
	...
	void SetColVal( int iColNum, STRINGT strValPARA )
	{
		...
		if( iColNum == 7 )
		{
			wl::SStrf::s2b(strValPARA,m_StaNode);
		}
		if( iColNum == 8 )
		{
...
private:
	...
	a3006_t_rowtype m_EmptyRow;
	std::vector<a3006_t_rowtype>  m_DATAcorpora;
 
public:

	typedef a3006_t_rowtype 	ROWTYPE;
	typedef std::vector<long> 	RPSTYPE; //RPS(return pointer set) type definition
	typedef std::vector<a3006_t_rowtype>::iterator 	TBLITTYPE;
...
public:
	...
	//operator == 
	bool operator == (const a3006_t & rhs) const
	{
		return this == &rhs;
	}
...
public:
	...
	a3006_t &  Unserialize( const char * strIn , int biAppend = 0 ) //if append==0, it would over-write the current tbl. 
	{
		char *p1,*p2;
		p1 = (char*)strIn;
		if(biAppend==0) Clear();
	...
	a3006_t & Unserialize( wl::SCake & ckIn , int biAppend = 0 )
	{
		char *p1,*p2;
		p1 = p2 = (char*)ckIn.buf();
		if(biAppend==0) Clear();
...
//Return:	a3006_t_rowtype&
	a3006_t_rowtype & GetRow(long lRowNum)
	{
		if(lRowNum>=0&&lRowNum<(long)m_DATAcorpora.size())
		return m_DATAcorpora[lRowNum];
		else 
		{
			a3006_t_rowtype tmpEmptyrow;
...
//Return:	a3006_t_rowtype&
	a3006_t_rowtype & GetRow(const std::vector<long> & vRps, long lRowNum)
	{
		long ltmp;
		do{
		{
...
//Return:	no return
	void Add(const a3006_t_rowtype & varRow)
	{
...
//Return:	a3006_t_rowtype&
	a3006_t_rowtype & SelE1_strTblName(std::string strVal) 
	{
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3006_t_rowtype&
	a3006_t_rowtype & SelE1_lVer(long iVal) 
	{
		std::vector<long> vRps ;
		SelE_lVer( iVal, vRps );
		return GetRow( vRps, 0 );
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3006_t_rowtype&
	a3006_t_rowtype & SelE1_strAffectDateTime(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3006_t_rowtype&
	a3006_t_rowtype & SelE1_biDelFlag(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3006_t_rowtype&
	a3006_t_rowtype & SelE1_biIsAffect(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3006_t_rowtype&
	a3006_t_rowtype & SelE1_strStampDateTime(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3006_t_rowtype&
	a3006_t_rowtype & SelE1_strDataRef(std::string strVal) 
	{
		std::vector<long> vRps ;
...
////////////////////////////////////////////////////////////////
//Function:	SelE_StaNode
//Effect:	select and build RPS where col=val use traverse
//Return:	no return
	void SelE_StaNode(u8arr_t<4> aVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //(ni)
	{
		for(long ltmp=0;ltmp<(long)m_DATAcorpora.size();ltmp++)
		if(m_DATAcorpora[ltmp].m_StaNode==aVal)
		if( !pRefRps || !pRefRps->empty() && std::binary_search( pRefRps->begin(), pRefRps->end(), ltmp ) ) vRps.push_back( ltmp );
	}

/////////////////////////////////////////////////////////////////
//Function:	SelE1_StaNode
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3006_t_rowtype&
	a3006_t_rowtype & SelE1_StaNode(u8arr_t<4> aVal) 
	{
		std::vector<long> vRps ;
		SelE_StaNode( aVal, vRps );
		return GetRow( vRps, 0 );
	}

/////////////////////////////////////////////////////////////////
//Function:	SelEc_StaNode
//Effect:	select and build RPS where col=val. clear rps first.
//Return:	no return
	void SelEc_StaNode(u8arr_t<4> aVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //()
	{
		vRps.clear();
		SelE_StaNode(aVal, vRps, pRefRps);
	}
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3006_t_rowtype&
	a3006_t_rowtype & SelE1_strStaNameEn(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3006_t_rowtype&
	a3006_t_rowtype & SelE1_lStaNameCh(long iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3006_t_rowtype&
	a3006_t_rowtype & SelE1_RES_01(u8arr_t<4> aVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3006_t_rowtype&
	a3006_t_rowtype & SelE1_RES_02(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
