...
#ifndef V1_3AA3011_T_20140523_161724
#define V1_3AA3011_T_20140523_161724

 
///////////////////////////////////////////////////////
//Contents:	The row class type definition of a3011_t
//Name:	a3011_t_rowtype
class a3011_t_rowtype {
 
public:

	std::string		m_strTblName;			// 1		2. 3011-清分系统本地语言资源文件。此类交互用于向需要用本地语言显示信息的节点 
	long		m_lVer;			// 2		参数版本 
	...
	long		m_lResPointer;			// a	1	 数据块pointer 
	std::string		m_strResBlockOri;			// a	2	 数据块，原始数据 
	std::string		m_strResBlockSys;			// a	3	 数据块，当前系统所需数据 
	u8arr_t<4>		m_RES_01;			// a	5	[保留]  Byte 4 
	wl::tuint8		m_RES_02;			// a	6	[保留]  Byte 1 
	wl::tuint8		m_RES_03;			// a	7	[保留] 
...
public:
	...
	virtual ~a3011_t_rowtype(){;}
	

	//operator == 
	bool operator == (const a3011_t_rowtype & rhs) const
	{
		return this == &rhs;
	}
...
public:
	...
	std::string & Serialize( std::string & strOut )
	{
		...
		strOut += std::string(buf2);
		strOut += std::string("/");
		buf1 = (const char *)reinterpret_cast<char *>(&reinterpret_cast<char&>(m_lResPointer));
		len1 = sizeof(m_lResPointer);
		if( (int)v.size() < ( len1 * 2 + 4 ) ) v.resize( len1 * 2 + 4 );
		buf2 = (char*)(&(v[0]));
		en( buf1, len1, buf2 );
		strOut += std::string(buf2);
		strOut += std::string("/");
		buf1 = (const char *)m_strResBlockOri.c_str();
		len1 = (long)(m_strResBlockOri.length()*sizeof(char)+sizeof(char));
		if( (int)v.size() < ( len1 * 2 + 4 ) ) v.resize( len1 * 2 + 4 );
		buf2 = (char*)(&(v[0]));
		en( buf1, len1, buf2 );
		strOut += std::string(buf2);
		strOut += std::string("/");
		buf1 = (const char *)m_strResBlockSys.c_str();
		len1 = (long)(m_strResBlockSys.length()*sizeof(char)+sizeof(char));
		if( (int)v.size() < ( len1 * 2 + 4 ) ) v.resize( len1 * 2 + 4 );
		buf2 = (char*)(&(v[0]));
		en( buf1, len1, buf2 );
		strOut += std::string(buf2);
	...
	a3011_t_rowtype & Unserialize( const char * strIn )
	{
		const char *buf1;
		char *buf2;
		buf1 = strIn;
		...
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
		 /*if( *buf2!='}')*/ m_lResPointer = *(long*)buf2; }else return *this; 
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
		 /*if( *buf2!='}')*/ m_strResBlockOri = (char*)buf2; }else return *this; 
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
		 /*if( *buf2!='}')*/ m_strResBlockSys = (char*)buf2; }else return *this; 
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
	...
	a3011_t_rowtype & Unserialize( wl::tchar * p_begin, wl::tchar ** pp_end )
	{
		char *buf1 = p_begin;
		char *buf2;
		buf2 = (char*)buf1 + 4;
		...
		buf2 = (char*)buf1 + 4;
		buf1 = buf2 + *(wl::tuint32*)buf1;
		m_lResPointer = *(long*)buf2;
		buf2 = (char*)buf1 + 4;
		buf1 = buf2 + *(wl::tuint32*)buf1;
		m_strResBlockOri = (char*)buf2;
		buf2 = (char*)buf1 + 4;
		buf1 = buf2 + *(wl::tuint32*)buf1;
		m_strResBlockSys = (char*)buf2;
		buf2 = (char*)buf1 + 4;
		buf1 = buf2 + *(wl::tuint32*)buf1;
...
////////////////////////////////////////////////////////////////
//Function:	GetCol_lResPointer
//Effect:	get ref of col. col is lResPointer
//Return:	long & 
	long & GetCol_lResPointer(void)
	{
		return m_lResPointer;
	}

///////////////////////////////////////////////////////////////
//Function:	GetCol_strResBlockOri
//Effect:	get ref of col. col is strResBlockOri
//Return:	std::string & 
	std::string & GetCol_strResBlockOri(void)
	{
		return m_strResBlockOri;
	}

////////////////////////////////////////////////////////
//Function:	GetCol_strResBlockSys
//Effect:	get ref of col. col is strResBlockSys
//Return:	std::string & 
	std::string & GetCol_strResBlockSys(void)
	{
		return m_strResBlockSys;
	}
...
//Return:	return col's name in string format.
	std::string GetColName( int iColNum )
	{
		...
		if( iColNum == 8 )
		{
			return "strResBlockOri";
		}
		if( iColNum == 9 )
		{
			return "strResBlockSys";
		}
		if( iColNum == 10 )
		{
			return "RES_01";
...
//Return:	return the column number, int.
	int GetColNumber( std::string strColName ) 
	{
		...
		if( strColName == "lResPointer" )
		{
			return 7;
		}
		if( strColName == "strResBlockOri" )
		{
			return 8;
		}
		if( strColName == "strResBlockSys" )
		{
			return 9;
		}
...
//Return:	return col value in string format.
	...
	STRINGT GetColStr( int iColNum )
	{
		...
		if( iColNum == 7 )
		{
			return wl::SStrf::sltoa(m_lResPointer);
		}
		if( iColNum == 8 )
		{
			return m_strResBlockOri;
...
//Return:	no return.
	...
	void SetColVal( int iColNum, STRINGT strValPARA )
	{
		...
		if( iColNum == 7 )
		{
			m_lResPointer=wl::SStrf::satol(strValPARA);
		}
		if( iColNum == 8 )
		{
			m_strResBlockOri=strValPARA;
		}
		if( iColNum == 9 )
		{
			m_strResBlockSys=strValPARA;
		}
		if( iColNum == 10 )
		{
...
private:
	...
	a3011_t_rowtype m_EmptyRow;
	std::vector<a3011_t_rowtype>  m_DATAcorpora;
 
public:

	typedef a3011_t_rowtype 	ROWTYPE;
	typedef std::vector<long> 	RPSTYPE; //RPS(return pointer set) type definition
	typedef std::vector<a3011_t_rowtype>::iterator 	TBLITTYPE;
...
public:
	...
	//operator == 
	bool operator == (const a3011_t & rhs) const
	{
		return this == &rhs;
	}
...
public:
	...
	a3011_t &  Unserialize( const char * strIn , int biAppend = 0 ) //if append==0, it would over-write the current tbl. 
	{
		char *p1,*p2;
		p1 = (char*)strIn;
		if(biAppend==0) Clear();
	...
	a3011_t & Unserialize( wl::SCake & ckIn , int biAppend = 0 )
	{
		char *p1,*p2;
		p1 = p2 = (char*)ckIn.buf();
		if(biAppend==0) Clear();
...
//Return:	a3011_t_rowtype&
	a3011_t_rowtype & GetRow(long lRowNum)
	{
		if(lRowNum>=0&&lRowNum<(long)m_DATAcorpora.size())
		return m_DATAcorpora[lRowNum];
		else 
		{
			a3011_t_rowtype tmpEmptyrow;
...
//Return:	a3011_t_rowtype&
	a3011_t_rowtype & GetRow(const std::vector<long> & vRps, long lRowNum)
	{
		long ltmp;
		do{
		{
...
//Return:	no return
	void Add(const a3011_t_rowtype & varRow)
	{
...
//Return:	a3011_t_rowtype&
	a3011_t_rowtype & SelE1_strTblName(std::string strVal) 
	{
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3011_t_rowtype&
	a3011_t_rowtype & SelE1_lVer(long iVal) 
	{
		std::vector<long> vRps ;
		SelE_lVer( iVal, vRps );
		return GetRow( vRps, 0 );
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3011_t_rowtype&
	a3011_t_rowtype & SelE1_strAffectDateTime(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3011_t_rowtype&
	a3011_t_rowtype & SelE1_biDelFlag(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3011_t_rowtype&
	a3011_t_rowtype & SelE1_biIsAffect(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3011_t_rowtype&
	a3011_t_rowtype & SelE1_strStampDateTime(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3011_t_rowtype&
	a3011_t_rowtype & SelE1_strDataRef(std::string strVal) 
	{
		std::vector<long> vRps ;
...
/////////////////////////////////////////////////////////////////
//Function:	SelE_lResPointer
//Effect:	select and build RPS where col=val use traverse
//Return:	no return
	void SelE_lResPointer(long iVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //(ni)
	{
		for(long ltmp=0;ltmp<(long)m_DATAcorpora.size();ltmp++)
		if(m_DATAcorpora[ltmp].m_lResPointer==iVal)
		if( !pRefRps || !pRefRps->empty() && std::binary_search( pRefRps->begin(), pRefRps->end(), ltmp ) ) vRps.push_back( ltmp );
	}

/////////////////////////////////////////////////////////////////
//Function:	SelE1_lResPointer
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3011_t_rowtype&
	a3011_t_rowtype & SelE1_lResPointer(long iVal) 
	{
		std::vector<long> vRps ;
		SelE_lResPointer( iVal, vRps );
		return GetRow( vRps, 0 );
	}

/////////////////////////////////////////////////////////////
//Function:	SelEc_lResPointer
//Effect:	select and build RPS where col=val. clear rps first.
//Return:	no return
	void SelEc_lResPointer(long iVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //()
	{
		vRps.clear();
		SelE_lResPointer(iVal, vRps, pRefRps);
	}

///////////////////////////////////////////////////////////
//Function:	SelE_strResBlockOri
//Effect:	select and build RPS where col=val use traverse
//Return:	no return
	void SelE_strResBlockOri(std::string strVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //(ni)
	{
		for(long ltmp=0;ltmp<(long)m_DATAcorpora.size();ltmp++)
		if(m_DATAcorpora[ltmp].m_strResBlockOri==strVal)
		if( !pRefRps || !pRefRps->empty() && std::binary_search( pRefRps->begin(), pRefRps->end(), ltmp ) ) vRps.push_back( ltmp );
	}

/////////////////////////////////////////////////////////
//Function:	SelE1_strResBlockOri
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3011_t_rowtype&
	a3011_t_rowtype & SelE1_strResBlockOri(std::string strVal) 
	{
		std::vector<long> vRps ;
		SelE_strResBlockOri( strVal, vRps );
		return GetRow( vRps, 0 );
	}

//////////////////////////////////////////////////////////
//Function:	SelEc_strResBlockOri
//Effect:	select and build RPS where col=val. clear rps first.
//Return:	no return
	void SelEc_strResBlockOri(std::string strVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //()
	{
		vRps.clear();
		SelE_strResBlockOri(strVal, vRps, pRefRps);
	}

////////////////////////////////////////////////////////////////
//Function:	SelE_strResBlockSys
//Effect:	select and build RPS where col=val use traverse
//Return:	no return
	void SelE_strResBlockSys(std::string strVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //(ni)
	{
		for(long ltmp=0;ltmp<(long)m_DATAcorpora.size();ltmp++)
		if(m_DATAcorpora[ltmp].m_strResBlockSys==strVal)
		if( !pRefRps || !pRefRps->empty() && std::binary_search( pRefRps->begin(), pRefRps->end(), ltmp ) ) vRps.push_back( ltmp );
	}

///////////////////////////////////////////////////////
//Function:	SelE1_strResBlockSys
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3011_t_rowtype&
	a3011_t_rowtype & SelE1_strResBlockSys(std::string strVal) 
	{
		std::vector<long> vRps ;
		SelE_strResBlockSys( strVal, vRps );
		return GetRow( vRps, 0 );
	}

///////////////////////////////////////////////////////////
//Function:	SelEc_strResBlockSys
//Effect:	select and build RPS where col=val. clear rps first.
//Return:	no return
	void SelEc_strResBlockSys(std::string strVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //()
	{
		vRps.clear();
		SelE_strResBlockSys(strVal, vRps, pRefRps);
	}
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3011_t_rowtype&
	a3011_t_rowtype & SelE1_RES_01(u8arr_t<4> aVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3011_t_rowtype&
	a3011_t_rowtype & SelE1_RES_02(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a3011_t_rowtype&
	a3011_t_rowtype & SelE1_RES_03(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
