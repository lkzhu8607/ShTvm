...
#ifndef V1_3AB8706_T_20150303_164743
#define V1_3AB8706_T_20150303_164743

 
///////////////////////////////////////////////////////
//Contents:	The row class type definition of b8706_t
//Name:	b8706_t_rowtype
class b8706_t_rowtype {
 
public:

	std::string		m_strTblName;			// 1		滚动屏-b8706 
	long		m_lVer;			// 2		参数版本 
	...
	std::string		m_DispStr;			// a	6	内容 
	int		m_Counter1;			// a	7	计数器1 
	int		m_Counter2;			// a	8	计数器2 
...
public:
	...
	virtual ~b8706_t_rowtype(){;}
	

	//operator == 
	bool operator == (const b8706_t_rowtype & rhs) const
	{
		return this == &rhs;
	}
...
public:
	...
	std::string & Serialize( std::string & strOut )
	{
		...
		strOut += std::string(buf2);
		strOut += std::string("/");
		buf1 = (const char *)m_DispStr.c_str();
		len1 = (long)(m_DispStr.length()*sizeof(char)+sizeof(char));
		if( (int)v.size() < ( len1 * 2 + 4 ) ) v.resize( len1 * 2 + 4 );
		buf2 = (char*)(&(v[0]));
		en( buf1, len1, buf2 );
		strOut += std::string(buf2);
		strOut += std::string("/");
		buf1 = (const char *)reinterpret_cast<char *>(&reinterpret_cast<char&>(m_Counter1));
		len1 = sizeof(m_Counter1);
		if( (int)v.size() < ( len1 * 2 + 4 ) ) v.resize( len1 * 2 + 4 );
		buf2 = (char*)(&(v[0]));
		en( buf1, len1, buf2 );
		strOut += std::string(buf2);
		strOut += std::string("/");
		buf1 = (const char *)reinterpret_cast<char *>(&reinterpret_cast<char&>(m_Counter2));
		len1 = sizeof(m_Counter2);
		if( (int)v.size() < ( len1 * 2 + 4 ) ) v.resize( len1 * 2 + 4 );
		buf2 = (char*)(&(v[0]));
		en( buf1, len1, buf2 );
		strOut += std::string(buf2);
	...
	b8706_t_rowtype & Unserialize( const char * strIn )
	{
		const char *buf1;
		char *buf2;
		buf1 = strIn;
		...
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
		 /*if( *buf2!='}')*/ m_DispStr = (char*)buf2; }else return *this; 
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
		 /*if( *buf2!='}')*/ m_Counter1 = *(int*)buf2; }else return *this; 
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
		 /*if( *buf2!='}')*/ m_Counter2 = *(int*)buf2; }else return *this; 
		return *this;
	}
	...
	b8706_t_rowtype & Unserialize( wl::tchar * p_begin, wl::tchar ** pp_end )
	{
		char *buf1 = p_begin;
		char *buf2;
		buf2 = (char*)buf1 + 4;
...
////////////////////////////////////////////////////////////
//Function:	GetCol_DispStr
//Effect:	get ref of col. col is DispStr
//Return:	std::string & 
	std::string & GetCol_DispStr(void)
	{
		return m_DispStr;
	}

//////////////////////////////////////////////////////////
//Function:	GetCol_Counter1
//Effect:	get ref of col. col is Counter1
//Return:	int & 
	int & GetCol_Counter1(void)
	{
		return m_Counter1;
	}

///////////////////////////////////////////////////////
//Function:	GetCol_Counter2
//Effect:	get ref of col. col is Counter2
//Return:	int & 
	int & GetCol_Counter2(void)
	{
		return m_Counter2;
	}
...
//Return:	return the column number, int.
	int GetColNumber( std::string strColName ) 
	{
		...
		if( strColName == "DispStr" )
		{
			return 12;
		}
		if( strColName == "Counter1" )
		{
			return 13;
		}
		if( strColName == "Counter2" )
		{
			return 14;
		}
		return -1;
...
//Return:	return col value in string format.
	...
	STRINGT GetColStr( int iColNum )
	{
		...
		if( iColNum == 13 )
		{
			return wl::SStrf::sltoa(m_Counter1);
		}
		if( iColNum == 14 )
		{
			return wl::SStrf::sltoa(m_Counter2);
		}
...
//Return:	no return.
	...
	void SetColVal( int iColNum, STRINGT strValPARA )
	{
		...
		if( iColNum == 12 )
		{
			m_DispStr=strValPARA;
		}
		if( iColNum == 13 )
		{
			m_Counter1=wl::SStrf::satol(strValPARA);
		}
		if( iColNum == 14 )
		{
			m_Counter2=wl::SStrf::satol(strValPARA);
		}
	}
...
private:
	...
	b8706_t_rowtype m_EmptyRow;
	std::vector<b8706_t_rowtype>  m_DATAcorpora;
 
public:

	typedef b8706_t_rowtype 	ROWTYPE;
	typedef std::vector<long> 	RPSTYPE; //RPS(return pointer set) type definition
	typedef std::vector<b8706_t_rowtype>::iterator 	TBLITTYPE;
...
public:
	...
	//operator == 
	bool operator == (const b8706_t & rhs) const
	{
		return this == &rhs;
	}
...
public:
	...
	b8706_t &  Unserialize( const char * strIn , int biAppend = 0 ) //if append==0, it would over-write the current tbl. 
	{
		char *p1,*p2;
		p1 = (char*)strIn;
		if(biAppend==0) Clear();
	...
	b8706_t & Unserialize( wl::SCake & ckIn , int biAppend = 0 )
	{
		char *p1,*p2;
		p1 = p2 = (char*)ckIn.buf();
		if(biAppend==0) Clear();
...
//Return:	b8706_t_rowtype&
	b8706_t_rowtype & GetRow(long lRowNum)
	{
		if(lRowNum>=0&&lRowNum<(long)m_DATAcorpora.size())
		return m_DATAcorpora[lRowNum];
		else 
		{
			b8706_t_rowtype tmpEmptyrow;
...
//Return:	b8706_t_rowtype&
	b8706_t_rowtype & GetRow(const std::vector<long> & vRps, long lRowNum)
	{
		long ltmp;
		do{
		{
...
//Return:	no return
	void Add(const b8706_t_rowtype & varRow)
	{
...
//Return:	b8706_t_rowtype&
	b8706_t_rowtype & SelE1_strTblName(std::string strVal) 
	{
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8706_t_rowtype&
	b8706_t_rowtype & SelE1_lVer(long iVal) 
	{
		std::vector<long> vRps ;
		SelE_lVer( iVal, vRps );
		return GetRow( vRps, 0 );
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8706_t_rowtype&
	b8706_t_rowtype & SelE1_strAffectDateTime(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8706_t_rowtype&
	b8706_t_rowtype & SelE1_biDelFlag(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8706_t_rowtype&
	b8706_t_rowtype & SelE1_biIsAffect(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8706_t_rowtype&
	b8706_t_rowtype & SelE1_strStampDateTime(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8706_t_rowtype&
	b8706_t_rowtype & SelE1_strDataRef(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8706_t_rowtype&
	b8706_t_rowtype & SelE1_ConnState(int iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8706_t_rowtype&
	b8706_t_rowtype & SelE1_RtnVal(int iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8706_t_rowtype&
	b8706_t_rowtype & SelE1_BigErr(int iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8706_t_rowtype&
	b8706_t_rowtype & SelE1_Major(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8706_t_rowtype&
	b8706_t_rowtype & SelE1_Minor(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//////////////////////////////////////////////////////////////
//Function:	SelE_DispStr
//Effect:	select and build RPS where col=val use traverse
//Return:	no return
	void SelE_DispStr(std::string strVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //(ni)
	{
		for(long ltmp=0;ltmp<(long)m_DATAcorpora.size();ltmp++)
		if(m_DATAcorpora[ltmp].m_DispStr==strVal)
		if( !pRefRps || !pRefRps->empty() && std::binary_search( pRefRps->begin(), pRefRps->end(), ltmp ) ) vRps.push_back( ltmp );
	}

////////////////////////////////////////////////////////////////
//Function:	SelE1_DispStr
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8706_t_rowtype&
	b8706_t_rowtype & SelE1_DispStr(std::string strVal) 
	{
		std::vector<long> vRps ;
		SelE_DispStr( strVal, vRps );
		return GetRow( vRps, 0 );
	}

//////////////////////////////////////////////////////////////
//Function:	SelEc_DispStr
//Effect:	select and build RPS where col=val. clear rps first.
//Return:	no return
	void SelEc_DispStr(std::string strVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //()
	{
		vRps.clear();
		SelE_DispStr(strVal, vRps, pRefRps);
	}

////////////////////////////////////////////////////////////
//Function:	SelE_Counter1
//Effect:	select and build RPS where col=val use traverse
//Return:	no return
	void SelE_Counter1(int iVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //(ni)
	{
		for(long ltmp=0;ltmp<(long)m_DATAcorpora.size();ltmp++)
		if(m_DATAcorpora[ltmp].m_Counter1==iVal)
		if( !pRefRps || !pRefRps->empty() && std::binary_search( pRefRps->begin(), pRefRps->end(), ltmp ) ) vRps.push_back( ltmp );
	}

/////////////////////////////////////////////////////////
//Function:	SelE1_Counter1
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8706_t_rowtype&
	b8706_t_rowtype & SelE1_Counter1(int iVal) 
	{
		std::vector<long> vRps ;
		SelE_Counter1( iVal, vRps );
		return GetRow( vRps, 0 );
	}

///////////////////////////////////////////////////////////////
//Function:	SelEc_Counter1
//Effect:	select and build RPS where col=val. clear rps first.
//Return:	no return
	void SelEc_Counter1(int iVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //()
	{
		vRps.clear();
		SelE_Counter1(iVal, vRps, pRefRps);
	}

////////////////////////////////////////////////////////////
//Function:	SelE_Counter2
//Effect:	select and build RPS where col=val use traverse
//Return:	no return
	void SelE_Counter2(int iVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //(ni)
	{
		for(long ltmp=0;ltmp<(long)m_DATAcorpora.size();ltmp++)
		if(m_DATAcorpora[ltmp].m_Counter2==iVal)
		if( !pRefRps || !pRefRps->empty() && std::binary_search( pRefRps->begin(), pRefRps->end(), ltmp ) ) vRps.push_back( ltmp );
	}

////////////////////////////////////////////////////////////
//Function:	SelE1_Counter2
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8706_t_rowtype&
	b8706_t_rowtype & SelE1_Counter2(int iVal) 
	{
		std::vector<long> vRps ;
		SelE_Counter2( iVal, vRps );
		return GetRow( vRps, 0 );
	}

/////////////////////////////////////////////////////////////////
//Function:	SelEc_Counter2
//Effect:	select and build RPS where col=val. clear rps first.
//Return:	no return
	void SelEc_Counter2(int iVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //()
	{
		vRps.clear();
		SelE_Counter2(iVal, vRps, pRefRps);
	}
