...
public:
	...
	T eat_data( tbool biTurnEndian = 1 , tbool biMoveOn = 1 )
	{
		...
		SStrf::mkData( m_ckBufData.buf() + m_iBufOffset, i ); 
...
public:
	...
	virtual tbool ut_SeriTblFn( std::string strFn ) = 0;
	virtual unitbl_base_t & ut_UnseriTblFn( std::string strFn ) = 0;
...
public:
	...
	virtual tbool ut_SeriTblFn( std::string strFn )
	{
		SFile fl;
		fl.bind(strFn);
		return this->ut_SeriTbl(fl);
	}

	virtual unitbl_base_t & ut_UnseriTblFn( std::string strFn )
	{
		...
		return this->ut_UnseriTbl( strContent, 0 );
	}
...
public:
	...
	tbool Load()
	{
		...
		this->ut_ClearTbl();
		rc = fl.read_str( strContent );
		this->ut_UnseriTbl( strContent, 0 );
...
private:
	...
	void load_profile()
	{
		...
		for( int i = 0; i < (int)vecSrcFileList.size(); i++ )
		{
			...
			SStrvs::vsa_imp(vecSrcFileList[i],std::string("_"),1,vecFileName);
			if(vecFileName.size() != 3 || strlen(vecFileName[0].c_str()) != 6 || 
				strlen(vecFileName[1].c_str()) != 8 )
			{
				SFile::erase(m_strWorkPath + vecSrcFileList[i]);
				continue;
			}

			std::string dir = SFile::GetDir(m_strWorkPath);

			std::vector<std::string > vfilename;
			SStrvs::vsa_imp(vecFileName[2],std::string("."),1,vfilename);
			if(vfilename.size() < 2)
			{
				SFile::erase(m_strWorkPath + vecSrcFileList[i]);
				continue;
			}
			if( vfilename[0] != SFile::GetOnlyname(dir))
			{
				SFile::erase(m_strWorkPath + vecSrcFileList[i]);
			}
			else
