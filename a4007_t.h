...
#ifndef V1_3AA4007_T_20140523_163230
#define V1_3AA4007_T_20140523_163230

 
///////////////////////////////////////////////////////
//Contents:	The row class type definition of a4007_t
//Name:	a4007_t_rowtype
class a4007_t_rowtype {
 
public:

	std::string		m_strTblName;			// 1		8.0.9  4007-车票黑名单-全量。此类交互应用于向交互的接受方传送黑名单车票的序 
	long		m_lVer;			// 2		参数版本 
...
public:
	...
	virtual ~a4007_t_rowtype(){;}
	

	//operator == 
	bool operator == (const a4007_t_rowtype & rhs) const
	{
		return this == &rhs;
	}
...
public:
	...
	a4007_t_rowtype & Unserialize( const char * strIn )
	{
		const char *buf1;
		char *buf2;
		buf1 = strIn;
	...
	a4007_t_rowtype & Unserialize( wl::tchar * p_begin, wl::tchar ** pp_end )
	{
		char *buf1 = p_begin;
		char *buf2;
		buf2 = (char*)buf1 + 4;
...
private:
	...
	a4007_t_rowtype m_EmptyRow;
	std::vector<a4007_t_rowtype>  m_DATAcorpora;
 
public:

	typedef a4007_t_rowtype 	ROWTYPE;
	typedef std::vector<long> 	RPSTYPE; //RPS(return pointer set) type definition
	typedef std::vector<a4007_t_rowtype>::iterator 	TBLITTYPE;
...
public:
	...
	//operator == 
	bool operator == (const a4007_t & rhs) const
	{
		return this == &rhs;
	}
...
public:
	...
	a4007_t &  Unserialize( const char * strIn , int biAppend = 0 ) //if append==0, it would over-write the current tbl. 
	{
		char *p1,*p2;
		p1 = (char*)strIn;
		if(biAppend==0) Clear();
	...
	a4007_t & Unserialize( wl::SCake & ckIn , int biAppend = 0 )
	{
		char *p1,*p2;
		p1 = p2 = (char*)ckIn.buf();
		if(biAppend==0) Clear();
...
//Return:	a4007_t_rowtype&
	a4007_t_rowtype & GetRow(long lRowNum)
	{
		if(lRowNum>=0&&lRowNum<(long)m_DATAcorpora.size())
		return m_DATAcorpora[lRowNum];
		else 
		{
			a4007_t_rowtype tmpEmptyrow;
...
//Return:	a4007_t_rowtype&
	a4007_t_rowtype & GetRow(const std::vector<long> & vRps, long lRowNum)
	{
		long ltmp;
		do{
		{
...
//Return:	no return
	void Add(const a4007_t_rowtype & varRow)
	{
...
//Return:	a4007_t_rowtype&
	a4007_t_rowtype & SelE1_strTblName(std::string strVal) 
	{
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4007_t_rowtype&
	a4007_t_rowtype & SelE1_lVer(long iVal) 
	{
		std::vector<long> vRps ;
		SelE_lVer( iVal, vRps );
		return GetRow( vRps, 0 );
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4007_t_rowtype&
	a4007_t_rowtype & SelE1_strAffectDateTime(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4007_t_rowtype&
	a4007_t_rowtype & SelE1_biDelFlag(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4007_t_rowtype&
	a4007_t_rowtype & SelE1_biIsAffect(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4007_t_rowtype&
	a4007_t_rowtype & SelE1_strStampDateTime(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4007_t_rowtype&
	a4007_t_rowtype & SelE1_strDataRef(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4007_t_rowtype&
	a4007_t_rowtype & SelE1_RES01(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4007_t_rowtype&
	a4007_t_rowtype & SelE1_RES02(u8arr_t<2> aVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4007_t_rowtype&
	a4007_t_rowtype & SelE1_RES03(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
