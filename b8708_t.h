...
#ifndef V1_3AB8708_T_20150303_164854
#define V1_3AB8708_T_20150303_164854

 
///////////////////////////////////////////////////////
//Contents:	The row class type definition of b8708_t
//Name:	b8708_t_rowtype
class b8708_t_rowtype {
 
public:

	std::string		m_strTblName;			// 1		UPS-b8708 
	long		m_lVer;			// 2		参数版本 
...
public:
	...
	virtual ~b8708_t_rowtype(){;}
	

	//operator == 
	bool operator == (const b8708_t_rowtype & rhs) const
	{
		return this == &rhs;
	}
...
public:
	...
	std::string & Serialize( std::string & strOut )
	{
		...
		strOut += std::string(buf2);
		strOut += std::string("/");
		buf1 = (const char *)reinterpret_cast<char *>(&reinterpret_cast<char&>(m_IsUpsNoPower));
		len1 = sizeof(m_IsUpsNoPower);
		if( (int)v.size() < ( len1 * 2 + 4 ) ) v.resize( len1 * 2 + 4 );
		buf2 = (char*)(&(v[0]));
		en( buf1, len1, buf2 );
		strOut += std::string(buf2);
	...
	b8708_t_rowtype & Unserialize( const char * strIn )
	{
		const char *buf1;
		char *buf2;
		buf1 = strIn;
		...
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
		 /*if( *buf2!='}')*/ m_IsUpsNoPower = *(int*)buf2; }else return *this; 
		return *this;
	}
	...
	b8708_t_rowtype & Unserialize( wl::tchar * p_begin, wl::tchar ** pp_end )
	{
		char *buf1 = p_begin;
		char *buf2;
		buf2 = (char*)buf1 + 4;
...
////////////////////////////////////////////////////////////
//Function:	GetCol_IsUpsNoPower
//Effect:	get ref of col. col is IsUpsNoPower
//Return:	int & 
	int & GetCol_IsUpsNoPower(void)
	{
		return m_IsUpsNoPower;
	}
...
//Return:	return col value in string format.
	...
	STRINGT GetColStr( int iColNum )
	{
		...
		if( iColNum == 12 )
		{
			return wl::SStrf::sltoa(m_IsUpsNoPower);
		}
...
//Return:	no return.
	...
	void SetColVal( int iColNum, STRINGT strValPARA )
	{
		...
		if( iColNum == 12 )
		{
			m_IsUpsNoPower=wl::SStrf::satol(strValPARA);
		}
	}
...
private:
	...
	b8708_t_rowtype m_EmptyRow;
	std::vector<b8708_t_rowtype>  m_DATAcorpora;
 
public:

	typedef b8708_t_rowtype 	ROWTYPE;
	typedef std::vector<long> 	RPSTYPE; //RPS(return pointer set) type definition
	typedef std::vector<b8708_t_rowtype>::iterator 	TBLITTYPE;
...
public:
	...
	//operator == 
	bool operator == (const b8708_t & rhs) const
	{
		return this == &rhs;
	}
...
public:
	...
	b8708_t &  Unserialize( const char * strIn , int biAppend = 0 ) //if append==0, it would over-write the current tbl. 
	{
		char *p1,*p2;
		p1 = (char*)strIn;
		if(biAppend==0) Clear();
	...
	b8708_t & Unserialize( wl::SCake & ckIn , int biAppend = 0 )
	{
		char *p1,*p2;
		p1 = p2 = (char*)ckIn.buf();
		if(biAppend==0) Clear();
...
//Return:	b8708_t_rowtype&
	b8708_t_rowtype & GetRow(long lRowNum)
	{
		if(lRowNum>=0&&lRowNum<(long)m_DATAcorpora.size())
		return m_DATAcorpora[lRowNum];
		else 
		{
			b8708_t_rowtype tmpEmptyrow;
...
//Return:	b8708_t_rowtype&
	b8708_t_rowtype & GetRow(const std::vector<long> & vRps, long lRowNum)
	{
		long ltmp;
		do{
		{
...
//Return:	no return
	void Add(const b8708_t_rowtype & varRow)
	{
...
//Return:	b8708_t_rowtype&
	b8708_t_rowtype & SelE1_strTblName(std::string strVal) 
	{
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8708_t_rowtype&
	b8708_t_rowtype & SelE1_lVer(long iVal) 
	{
		std::vector<long> vRps ;
		SelE_lVer( iVal, vRps );
		return GetRow( vRps, 0 );
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8708_t_rowtype&
	b8708_t_rowtype & SelE1_strAffectDateTime(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8708_t_rowtype&
	b8708_t_rowtype & SelE1_biDelFlag(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8708_t_rowtype&
	b8708_t_rowtype & SelE1_biIsAffect(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8708_t_rowtype&
	b8708_t_rowtype & SelE1_strStampDateTime(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8708_t_rowtype&
	b8708_t_rowtype & SelE1_strDataRef(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8708_t_rowtype&
	b8708_t_rowtype & SelE1_ConnState(int iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8708_t_rowtype&
	b8708_t_rowtype & SelE1_RtnVal(int iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8708_t_rowtype&
	b8708_t_rowtype & SelE1_BigErr(int iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8708_t_rowtype&
	b8708_t_rowtype & SelE1_Major(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8708_t_rowtype&
	b8708_t_rowtype & SelE1_Minor(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//////////////////////////////////////////////////////////
//Function:	SelE_IsUpsNoPower
//Effect:	select and build RPS where col=val use traverse
//Return:	no return
	void SelE_IsUpsNoPower(int iVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //(ni)
	{
		for(long ltmp=0;ltmp<(long)m_DATAcorpora.size();ltmp++)
		if(m_DATAcorpora[ltmp].m_IsUpsNoPower==iVal)
		if( !pRefRps || !pRefRps->empty() && std::binary_search( pRefRps->begin(), pRefRps->end(), ltmp ) ) vRps.push_back( ltmp );
	}

/////////////////////////////////////////////////////////////
//Function:	SelE1_IsUpsNoPower
//Effect:	select 1st row where col=val, or return default row.
//Return:	b8708_t_rowtype&
	b8708_t_rowtype & SelE1_IsUpsNoPower(int iVal) 
	{
		std::vector<long> vRps ;
		SelE_IsUpsNoPower( iVal, vRps );
		return GetRow( vRps, 0 );
	}

//////////////////////////////////////////////////////////////
//Function:	SelEc_IsUpsNoPower
//Effect:	select and build RPS where col=val. clear rps first.
//Return:	no return
	void SelEc_IsUpsNoPower(int iVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //()
	{
		vRps.clear();
		SelE_IsUpsNoPower(iVal, vRps, pRefRps);
	}
