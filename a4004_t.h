...
#ifndef V1_3AA4004_T_20140523_163038
#define V1_3AA4004_T_20140523_163038

 
///////////////////////////////////////////////////////
//Contents:	The row class type definition of a4004_t
//Name:	a4004_t_rowtype
class a4004_t_rowtype {
 
public:

	std::string		m_strTblName;			// 1		8.0.6  4004-区域表-出站检票机。此类交互应用于向交互的接受方发送区域参数。本 
	long		m_lVer;			// 2		参数版本 
	...
	u8arr_t<4>		m_ANode;			// a	1	 车站标识码 SC节点标识码			Block 4 
	u8arr_t<4>		m_BNode;			// a	2	   目标车站标识码 SC节点标识码			Block 4 
	long		m_lZone;			// a	3	区域码 区域索引0-区域1……63-区域64 				Byte 1 
...
public:
	...
	virtual ~a4004_t_rowtype(){;}
	

	//operator == 
	bool operator == (const a4004_t_rowtype & rhs) const
	{
		return this == &rhs;
	}
...
public:
	...
	std::string & Serialize( std::string & strOut )
	{
		...
		strOut += std::string(buf2);
		strOut += std::string("/");
		buf1 = (const char *)reinterpret_cast<char *>(&reinterpret_cast<char&>(m_ANode));
		len1 = sizeof(m_ANode);
		if( (int)v.size() < ( len1 * 2 + 4 ) ) v.resize( len1 * 2 + 4 );
		buf2 = (char*)(&(v[0]));
		en( buf1, len1, buf2 );
		strOut += std::string(buf2);
		strOut += std::string("/");
		buf1 = (const char *)reinterpret_cast<char *>(&reinterpret_cast<char&>(m_BNode));
		len1 = sizeof(m_BNode);
		if( (int)v.size() < ( len1 * 2 + 4 ) ) v.resize( len1 * 2 + 4 );
		buf2 = (char*)(&(v[0]));
		en( buf1, len1, buf2 );
		strOut += std::string(buf2);
		strOut += std::string("/");
		buf1 = (const char *)reinterpret_cast<char *>(&reinterpret_cast<char&>(m_lZone));
		len1 = sizeof(m_lZone);
		if( (int)v.size() < ( len1 * 2 + 4 ) ) v.resize( len1 * 2 + 4 );
		buf2 = (char*)(&(v[0]));
		en( buf1, len1, buf2 );
	...
	a4004_t_rowtype & Unserialize( const char * strIn )
	{
		const char *buf1;
		char *buf2;
		buf1 = strIn;
		...
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
		 /*if( *buf2!='}')*/ m_ANode = *(u8arr_t<4>*)buf2; }else return *this; 
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
		 /*if( *buf2!='}')*/ m_BNode = *(u8arr_t<4>*)buf2; }else return *this; 
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
		 /*if( *buf2!='}')*/ m_lZone = *(long*)buf2; }else return *this; 
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
	...
	a4004_t_rowtype & Unserialize( wl::tchar * p_begin, wl::tchar ** pp_end )
	{
		char *buf1 = p_begin;
		char *buf2;
		buf2 = (char*)buf1 + 4;
		...
		buf2 = (char*)buf1 + 4;
		buf1 = buf2 + *(wl::tuint32*)buf1;
		m_ANode = *(u8arr_t<4>*)buf2;
		buf2 = (char*)buf1 + 4;
		buf1 = buf2 + *(wl::tuint32*)buf1;
		m_BNode = *(u8arr_t<4>*)buf2;
		buf2 = (char*)buf1 + 4;
		buf1 = buf2 + *(wl::tuint32*)buf1;
		m_lZone = *(long*)buf2;
		buf2 = (char*)buf1 + 4;
...
////////////////////////////////////////////////////////////////
//Function:	GetCol_ANode
//Effect:	get ref of col. col is ANode
//Return:	u8arr_t<4> & 
	u8arr_t<4> & GetCol_ANode(void)
	{
		return m_ANode;
	}

///////////////////////////////////////////////////////////////
//Function:	GetCol_BNode
//Effect:	get ref of col. col is BNode
//Return:	u8arr_t<4> & 
	u8arr_t<4> & GetCol_BNode(void)
	{
		return m_BNode;
	}

////////////////////////////////////////////////////////
//Function:	GetCol_lZone
//Effect:	get ref of col. col is lZone
//Return:	long & 
	long & GetCol_lZone(void)
	{
		return m_lZone;
	}
...
//Return:	return col value in string format.
	...
	STRINGT GetColStr( int iColNum )
	{
		...
		if( iColNum == 7 )
		{
			return wl::SStrf::b2s(m_ANode);
		}
		if( iColNum == 8 )
		{
			return wl::SStrf::b2s(m_BNode);
		}
		if( iColNum == 9 )
		{
			return wl::SStrf::sltoa(m_lZone);
		}
		if( iColNum == 10 )
		{
...
//Return:	no return.
	...
	void SetColVal( int iColNum, STRINGT strValPARA )
	{
		...
		if( iColNum == 7 )
		{
			wl::SStrf::s2b(strValPARA,m_ANode);
		}
		if( iColNum == 8 )
		{
			wl::SStrf::s2b(strValPARA,m_BNode);
		}
		if( iColNum == 9 )
		{
			m_lZone=wl::SStrf::satol(strValPARA);
		}
		if( iColNum == 10 )
		{
...
private:
	...
	a4004_t_rowtype m_EmptyRow;
	std::vector<a4004_t_rowtype>  m_DATAcorpora;
 
public:

	typedef a4004_t_rowtype 	ROWTYPE;
	typedef std::vector<long> 	RPSTYPE; //RPS(return pointer set) type definition
	typedef std::vector<a4004_t_rowtype>::iterator 	TBLITTYPE;
...
public:
	...
	//operator == 
	bool operator == (const a4004_t & rhs) const
	{
		return this == &rhs;
	}
...
public:
	...
	a4004_t &  Unserialize( const char * strIn , int biAppend = 0 ) //if append==0, it would over-write the current tbl. 
	{
		char *p1,*p2;
		p1 = (char*)strIn;
		if(biAppend==0) Clear();
	...
	a4004_t & Unserialize( wl::SCake & ckIn , int biAppend = 0 )
	{
		char *p1,*p2;
		p1 = p2 = (char*)ckIn.buf();
		if(biAppend==0) Clear();
...
//Return:	a4004_t_rowtype&
	a4004_t_rowtype & GetRow(long lRowNum)
	{
		if(lRowNum>=0&&lRowNum<(long)m_DATAcorpora.size())
		return m_DATAcorpora[lRowNum];
		else 
		{
			a4004_t_rowtype tmpEmptyrow;
...
//Return:	a4004_t_rowtype&
	a4004_t_rowtype & GetRow(const std::vector<long> & vRps, long lRowNum)
	{
		long ltmp;
		do{
		{
...
//Return:	no return
	void Add(const a4004_t_rowtype & varRow)
	{
...
//Return:	a4004_t_rowtype&
	a4004_t_rowtype & SelE1_strTblName(std::string strVal) 
	{
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4004_t_rowtype&
	a4004_t_rowtype & SelE1_lVer(long iVal) 
	{
		std::vector<long> vRps ;
		SelE_lVer( iVal, vRps );
		return GetRow( vRps, 0 );
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4004_t_rowtype&
	a4004_t_rowtype & SelE1_strAffectDateTime(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4004_t_rowtype&
	a4004_t_rowtype & SelE1_biDelFlag(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4004_t_rowtype&
	a4004_t_rowtype & SelE1_biIsAffect(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4004_t_rowtype&
	a4004_t_rowtype & SelE1_strStampDateTime(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4004_t_rowtype&
	a4004_t_rowtype & SelE1_strDataRef(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select and build RPS where col=val use traverse
//Return:	no return
	void SelE_ANode(u8arr_t<4> aVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //(ni)
	{
		for(long ltmp=0;ltmp<(long)m_DATAcorpora.size();ltmp++)
		if(m_DATAcorpora[ltmp].m_ANode==aVal)
		if( !pRefRps || !pRefRps->empty() && std::binary_search( pRefRps->begin(), pRefRps->end(), ltmp ) ) vRps.push_back( ltmp );
	}

/////////////////////////////////////////////////////////////////
//Function:	SelE1_ANode
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4004_t_rowtype&
	a4004_t_rowtype & SelE1_ANode(u8arr_t<4> aVal) 
	{
		std::vector<long> vRps ;
		SelE_ANode( aVal, vRps );
		return GetRow( vRps, 0 );
	}

/////////////////////////////////////////////////////////////
//Function:	SelEc_ANode
//Effect:	select and build RPS where col=val. clear rps first.
//Return:	no return
	void SelEc_ANode(u8arr_t<4> aVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //()
	{
		vRps.clear();
		SelE_ANode(aVal, vRps, pRefRps);
	}
...
//Effect:	select and build RPS where col=val use traverse
//Return:	no return
	void SelE_BNode(u8arr_t<4> aVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //(ni)
	{
		for(long ltmp=0;ltmp<(long)m_DATAcorpora.size();ltmp++)
		if(m_DATAcorpora[ltmp].m_BNode==aVal)
		if( !pRefRps || !pRefRps->empty() && std::binary_search( pRefRps->begin(), pRefRps->end(), ltmp ) ) vRps.push_back( ltmp );
	}

/////////////////////////////////////////////////////////
//Function:	SelE1_BNode
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4004_t_rowtype&
	a4004_t_rowtype & SelE1_BNode(u8arr_t<4> aVal) 
	{
		std::vector<long> vRps ;
		SelE_BNode( aVal, vRps );
		return GetRow( vRps, 0 );
	}

//////////////////////////////////////////////////////////
//Function:	SelEc_BNode
//Effect:	select and build RPS where col=val. clear rps first.
//Return:	no return
	void SelEc_BNode(u8arr_t<4> aVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //()
	{
		vRps.clear();
		SelE_BNode(aVal, vRps, pRefRps);
	}
...
//Effect:	select and build RPS where col=val use traverse
//Return:	no return
	void SelE_lZone(long iVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //(ni)
	{
		for(long ltmp=0;ltmp<(long)m_DATAcorpora.size();ltmp++)
		if(m_DATAcorpora[ltmp].m_lZone==iVal)
		if( !pRefRps || !pRefRps->empty() && std::binary_search( pRefRps->begin(), pRefRps->end(), ltmp ) ) vRps.push_back( ltmp );
	}

///////////////////////////////////////////////////////
//Function:	SelE1_lZone
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4004_t_rowtype&
	a4004_t_rowtype & SelE1_lZone(long iVal) 
	{
		std::vector<long> vRps ;
		SelE_lZone( iVal, vRps );
		return GetRow( vRps, 0 );
	}

///////////////////////////////////////////////////////////
//Function:	SelEc_lZone
//Effect:	select and build RPS where col=val. clear rps first.
//Return:	no return
	void SelEc_lZone(long iVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //()
	{
		vRps.clear();
		SelE_lZone(iVal, vRps, pRefRps);
	}
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4004_t_rowtype&
	a4004_t_rowtype & SelE1_RES01(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4004_t_rowtype&
	a4004_t_rowtype & SelE1_RES02(u8arr_t<2> aVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4004_t_rowtype&
	a4004_t_rowtype & SelE1_RES03(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
