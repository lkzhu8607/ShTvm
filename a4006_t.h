...
#ifndef V1_3AA4006_T_20140523_163147
#define V1_3AA4006_T_20140523_163147

 
///////////////////////////////////////////////////////
//Contents:	The row class type definition of a4006_t
//Name:	a4006_t_rowtype
class a4006_t_rowtype {
 
public:

	std::string		m_strTblName;			// 1		8.0.8  4006-非高峰时段表。此类交互应用于向交互的接受方发送非高峰时段参数。该 
	long		m_lVer;			// 2		参数版本 
	...
	longarr2v_t<7,96*3>		m_UnRushHourBeginTimeflag;			// a	1	时段 得知星期数，96个时间段，每个时段是15*60秒，备三份，取中间一份.值是0或1。 
	long		m_UnRushHourIdx;			// a	2	 非高峰时段表中记录的索引 
	wl::tuint8		m_RES01;			// a	3	[保留] 
	u8arr_t<2>		m_RES02;			// a	4	[保留] 
	wl::tuint8		m_RES03;			// a	5	[保留] 
...
public:
	...
	a4006_t_rowtype()
	{
		...
		m_strDataRef = "";
		wl::SStrf::smemset(m_UnRushHourBeginTimeflag);
		m_UnRushHourIdx = 0;
		m_RES01 = 0;
		//m_RES02 = //use default
		m_RES03 = 0;
	...
	virtual ~a4006_t_rowtype(){;}
	

	//operator == 
	bool operator == (const a4006_t_rowtype & rhs) const
	{
		return this == &rhs;
	}
...
public:
	...
	std::string & Serialize( std::string & strOut )
	{
		...
		strOut += std::string(buf2);
		strOut += std::string("/");
		buf1 = (const char *)reinterpret_cast<char *>(&reinterpret_cast<char&>(m_UnRushHourBeginTimeflag));
		len1 = sizeof(m_UnRushHourBeginTimeflag);
		if( (int)v.size() < ( len1 * 2 + 4 ) ) v.resize( len1 * 2 + 4 );
		buf2 = (char*)(&(v[0]));
		en( buf1, len1, buf2 );
		strOut += std::string(buf2);
		strOut += std::string("/");
		buf1 = (const char *)reinterpret_cast<char *>(&reinterpret_cast<char&>(m_UnRushHourIdx));
		len1 = sizeof(m_UnRushHourIdx);
		if( (int)v.size() < ( len1 * 2 + 4 ) ) v.resize( len1 * 2 + 4 );
		buf2 = (char*)(&(v[0]));
		en( buf1, len1, buf2 );
		strOut += std::string(buf2);
	...
	a4006_t_rowtype & Unserialize( const char * strIn )
	{
		const char *buf1;
		char *buf2;
		buf1 = strIn;
		...
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
		 /*if( *buf2!='}')*/ m_UnRushHourBeginTimeflag = *(longarr2v_t<7,96*3>*)buf2; }else return *this; 
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
		 /*if( *buf2!='}')*/ m_UnRushHourIdx = *(long*)buf2; }else return *this; 
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
	...
	a4006_t_rowtype & Unserialize( wl::tchar * p_begin, wl::tchar ** pp_end )
	{
		char *buf1 = p_begin;
		char *buf2;
		buf2 = (char*)buf1 + 4;
		...
		buf2 = (char*)buf1 + 4;
		buf1 = buf2 + *(wl::tuint32*)buf1;
		m_UnRushHourBeginTimeflag = *(longarr2v_t<7,96*3>*)buf2;
		buf2 = (char*)buf1 + 4;
		buf1 = buf2 + *(wl::tuint32*)buf1;
		m_UnRushHourIdx = *(long*)buf2;
		buf2 = (char*)buf1 + 4;
		buf1 = buf2 + *(wl::tuint32*)buf1;
...
////////////////////////////////////////////////////////////////
//Function:	GetCol_UnRushHourBeginTimeflag
//Effect:	get ref of col. col is UnRushHourBeginTimeflag
//Return:	longarr2v_t<7,96*3> & 
	longarr2v_t<7,96*3> & GetCol_UnRushHourBeginTimeflag(void)
	{
		return m_UnRushHourBeginTimeflag;
	}

///////////////////////////////////////////////////////////////
//Function:	GetCol_UnRushHourIdx
//Effect:	get ref of col. col is UnRushHourIdx
//Return:	long & 
	long & GetCol_UnRushHourIdx(void)
	{
		return m_UnRushHourIdx;
	}
...
//Return:	return col's name in string format.
	std::string GetColName( int iColNum )
	{
		...
		if( iColNum == 7 )
		{
			return "UnRushHourBeginTimeflag";
		}
		if( iColNum == 8 )
		{
			return "UnRushHourIdx";
		}
		if( iColNum == 9 )
		{
			return "RES01";
...
//Return:	return the column number, int.
	int GetColNumber( std::string strColName ) 
	{
		...
		if( strColName == "UnRushHourBeginTimeflag" )
		{
			return 7;
		}
		if( strColName == "UnRushHourIdx" )
		{
			return 8;
		}
		if( strColName == "RES01" )
...
//Return:	return col value in string format.
	...
	STRINGT GetColStr( int iColNum )
	{
		...
		if( iColNum == 7 )
		{
			return wl::SStrf::b2s(m_UnRushHourBeginTimeflag);
		}
		if( iColNum == 8 )
		{
			return wl::SStrf::sltoa(m_UnRushHourIdx);
		}
		if( iColNum == 9 )
		{
...
//Return:	no return.
	...
	void SetColVal( int iColNum, STRINGT strValPARA )
	{
		...
		if( iColNum == 7 )
		{
			wl::SStrf::s2b(strValPARA,m_UnRushHourBeginTimeflag);
		}
		if( iColNum == 8 )
		{
			m_UnRushHourIdx=wl::SStrf::satol(strValPARA);
		}
		if( iColNum == 9 )
		{
...
private:
	...
	a4006_t_rowtype m_EmptyRow;
	std::vector<a4006_t_rowtype>  m_DATAcorpora;
 
public:

	typedef a4006_t_rowtype 	ROWTYPE;
	typedef std::vector<long> 	RPSTYPE; //RPS(return pointer set) type definition
	typedef std::vector<a4006_t_rowtype>::iterator 	TBLITTYPE;
...
public:
	...
	//operator == 
	bool operator == (const a4006_t & rhs) const
	{
		return this == &rhs;
	}
...
public:
	...
	a4006_t &  Unserialize( const char * strIn , int biAppend = 0 ) //if append==0, it would over-write the current tbl. 
	{
		char *p1,*p2;
		p1 = (char*)strIn;
		if(biAppend==0) Clear();
	...
	a4006_t & Unserialize( wl::SCake & ckIn , int biAppend = 0 )
	{
		char *p1,*p2;
		p1 = p2 = (char*)ckIn.buf();
		if(biAppend==0) Clear();
...
//Return:	a4006_t_rowtype&
	a4006_t_rowtype & GetRow(long lRowNum)
	{
		if(lRowNum>=0&&lRowNum<(long)m_DATAcorpora.size())
		return m_DATAcorpora[lRowNum];
		else 
		{
			a4006_t_rowtype tmpEmptyrow;
...
//Return:	a4006_t_rowtype&
	a4006_t_rowtype & GetRow(const std::vector<long> & vRps, long lRowNum)
	{
		long ltmp;
		do{
		{
...
//Return:	no return
	void Add(const a4006_t_rowtype & varRow)
	{
...
//Return:	a4006_t_rowtype&
	a4006_t_rowtype & SelE1_strTblName(std::string strVal) 
	{
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4006_t_rowtype&
	a4006_t_rowtype & SelE1_lVer(long iVal) 
	{
		std::vector<long> vRps ;
		SelE_lVer( iVal, vRps );
		return GetRow( vRps, 0 );
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4006_t_rowtype&
	a4006_t_rowtype & SelE1_strAffectDateTime(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4006_t_rowtype&
	a4006_t_rowtype & SelE1_biDelFlag(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4006_t_rowtype&
	a4006_t_rowtype & SelE1_biIsAffect(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4006_t_rowtype&
	a4006_t_rowtype & SelE1_strStampDateTime(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4006_t_rowtype&
	a4006_t_rowtype & SelE1_strDataRef(std::string strVal) 
	{
		std::vector<long> vRps ;
...
////////////////////////////////////////////////////////////////
//Function:	SelE_UnRushHourBeginTimeflag
//Effect:	select and build RPS where col=val use traverse
//Return:	no return
	void SelE_UnRushHourBeginTimeflag(longarr2v_t<7,96*3> aVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //(ni)
	{
		for(long ltmp=0;ltmp<(long)m_DATAcorpora.size();ltmp++)
		if(m_DATAcorpora[ltmp].m_UnRushHourBeginTimeflag==aVal)
		if( !pRefRps || !pRefRps->empty() && std::binary_search( pRefRps->begin(), pRefRps->end(), ltmp ) ) vRps.push_back( ltmp );
	}

/////////////////////////////////////////////////////////////////
//Function:	SelE1_UnRushHourBeginTimeflag
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4006_t_rowtype&
	a4006_t_rowtype & SelE1_UnRushHourBeginTimeflag(longarr2v_t<7,96*3> aVal) 
	{
		std::vector<long> vRps ;
		SelE_UnRushHourBeginTimeflag( aVal, vRps );
		return GetRow( vRps, 0 );
	}

/////////////////////////////////////////////////////////////////
//Function:	SelEc_UnRushHourBeginTimeflag
//Effect:	select and build RPS where col=val. clear rps first.
//Return:	no return
	void SelEc_UnRushHourBeginTimeflag(longarr2v_t<7,96*3> aVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //()
	{
		vRps.clear();
		SelE_UnRushHourBeginTimeflag(aVal, vRps, pRefRps);
	}

/////////////////////////////////////////////////////////////
//Function:	SelE_UnRushHourIdx
//Effect:	select and build RPS where col=val use traverse
//Return:	no return
	void SelE_UnRushHourIdx(long iVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //(ni)
	{
		for(long ltmp=0;ltmp<(long)m_DATAcorpora.size();ltmp++)
		if(m_DATAcorpora[ltmp].m_UnRushHourIdx==iVal)
		if( !pRefRps || !pRefRps->empty() && std::binary_search( pRefRps->begin(), pRefRps->end(), ltmp ) ) vRps.push_back( ltmp );
	}

///////////////////////////////////////////////////////////
//Function:	SelE1_UnRushHourIdx
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4006_t_rowtype&
	a4006_t_rowtype & SelE1_UnRushHourIdx(long iVal) 
	{
		std::vector<long> vRps ;
		SelE_UnRushHourIdx( iVal, vRps );
		return GetRow( vRps, 0 );
	}

/////////////////////////////////////////////////////////
//Function:	SelEc_UnRushHourIdx
//Effect:	select and build RPS where col=val. clear rps first.
//Return:	no return
	void SelEc_UnRushHourIdx(long iVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //()
	{
		vRps.clear();
		SelE_UnRushHourIdx(iVal, vRps, pRefRps);
	}
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4006_t_rowtype&
	a4006_t_rowtype & SelE1_RES01(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4006_t_rowtype&
	a4006_t_rowtype & SelE1_RES02(u8arr_t<2> aVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4006_t_rowtype&
	a4006_t_rowtype & SelE1_RES03(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
