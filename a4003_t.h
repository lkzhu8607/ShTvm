...
#ifndef V1_3AA4003_T_20140523_162929
#define V1_3AA4003_T_20140523_162929

 
///////////////////////////////////////////////////////
//Contents:	The row class type definition of a4003_t
//Name:	a4003_t_rowtype
class a4003_t_rowtype {
 
public:

	std::string		m_strTblName;			// 1		8.0.5  4003-费率表。此类交互应用于向交互的接受方发送费率参数。这些参数被车票 
	long		m_lVer;			// 2		参数版本 
	...
	ui16arr_t<64>		m_FareRate;			// a	1	 区域1-64费率值 
	u8arr_t<64>		m_RES_01;			// a	2	[保留]  Word 64 
	long		m_lFareRateIdx;			// a	3	记录的索引 
...
public:
	...
	virtual ~a4003_t_rowtype(){;}
	

	//operator == 
	bool operator == (const a4003_t_rowtype & rhs) const
	{
		return this == &rhs;
	}
...
public:
	...
	std::string & Serialize( std::string & strOut )
	{
		...
		strOut += std::string(buf2);
		strOut += std::string("/");
		buf1 = (const char *)reinterpret_cast<char *>(&reinterpret_cast<char&>(m_FareRate));
		len1 = sizeof(m_FareRate);
		if( (int)v.size() < ( len1 * 2 + 4 ) ) v.resize( len1 * 2 + 4 );
		buf2 = (char*)(&(v[0]));
		en( buf1, len1, buf2 );
		strOut += std::string(buf2);
		...
		strOut += std::string(buf2);
		strOut += std::string("/");
		buf1 = (const char *)reinterpret_cast<char *>(&reinterpret_cast<char&>(m_lFareRateIdx));
		len1 = sizeof(m_lFareRateIdx);
		if( (int)v.size() < ( len1 * 2 + 4 ) ) v.resize( len1 * 2 + 4 );
		buf2 = (char*)(&(v[0]));
		en( buf1, len1, buf2 );
		strOut += std::string(buf2);
	...
	a4003_t_rowtype & Unserialize( const char * strIn )
	{
		const char *buf1;
		char *buf2;
		buf1 = strIn;
		...
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
		 /*if( *buf2!='}')*/ m_FareRate = *(ui16arr_t<64>*)buf2; }else return *this; 
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
		 /*if( *buf2!='}')*/ m_RES_01 = *(u8arr_t<64>*)buf2; }else return *this; 
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
		 /*if( *buf2!='}')*/ m_lFareRateIdx = *(long*)buf2; }else return *this; 
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
	...
	a4003_t_rowtype & Unserialize( wl::tchar * p_begin, wl::tchar ** pp_end )
	{
		char *buf1 = p_begin;
		char *buf2;
		buf2 = (char*)buf1 + 4;
		...
		buf2 = (char*)buf1 + 4;
		buf1 = buf2 + *(wl::tuint32*)buf1;
		m_FareRate = *(ui16arr_t<64>*)buf2;
		buf2 = (char*)buf1 + 4;
		buf1 = buf2 + *(wl::tuint32*)buf1;
		m_RES_01 = *(u8arr_t<64>*)buf2;
		buf2 = (char*)buf1 + 4;
		buf1 = buf2 + *(wl::tuint32*)buf1;
		m_lFareRateIdx = *(long*)buf2;
		buf2 = (char*)buf1 + 4;
		buf1 = buf2 + *(wl::tuint32*)buf1;
...
////////////////////////////////////////////////////////////////
//Function:	GetCol_FareRate
//Effect:	get ref of col. col is FareRate
//Return:	ui16arr_t<64> & 
	ui16arr_t<64> & GetCol_FareRate(void)
	{
		return m_FareRate;
	}
...
//Return:	u8arr_t<64> & 
	u8arr_t<64> & GetCol_RES_01(void)
	{
		return m_RES_01;
	}

////////////////////////////////////////////////////////
//Function:	GetCol_lFareRateIdx
//Effect:	get ref of col. col is lFareRateIdx
//Return:	long & 
	long & GetCol_lFareRateIdx(void)
	{
		return m_lFareRateIdx;
	}
...
//Return:	return col value in string format.
	...
	STRINGT GetColStr( int iColNum )
	{
		...
		if( iColNum == 7 )
		{
			return wl::SStrf::b2s(m_FareRate);
		}
		if( iColNum == 8 )
		{
		...
		if( iColNum == 9 )
		{
			return wl::SStrf::sltoa(m_lFareRateIdx);
		}
		if( iColNum == 10 )
		{
...
//Return:	no return.
	...
	void SetColVal( int iColNum, STRINGT strValPARA )
	{
		...
		if( iColNum == 7 )
		{
			wl::SStrf::s2b(strValPARA,m_FareRate);
		}
		if( iColNum == 8 )
		{
		...
		if( iColNum == 9 )
		{
			m_lFareRateIdx=wl::SStrf::satol(strValPARA);
		}
		if( iColNum == 10 )
		{
...
private:
	...
	a4003_t_rowtype m_EmptyRow;
	std::vector<a4003_t_rowtype>  m_DATAcorpora;
 
public:

	typedef a4003_t_rowtype 	ROWTYPE;
	typedef std::vector<long> 	RPSTYPE; //RPS(return pointer set) type definition
	typedef std::vector<a4003_t_rowtype>::iterator 	TBLITTYPE;
...
public:
	...
	//operator == 
	bool operator == (const a4003_t & rhs) const
	{
		return this == &rhs;
	}
...
public:
	...
	a4003_t &  Unserialize( const char * strIn , int biAppend = 0 ) //if append==0, it would over-write the current tbl. 
	{
		char *p1,*p2;
		p1 = (char*)strIn;
		if(biAppend==0) Clear();
	...
	a4003_t & Unserialize( wl::SCake & ckIn , int biAppend = 0 )
	{
		char *p1,*p2;
		p1 = p2 = (char*)ckIn.buf();
		if(biAppend==0) Clear();
...
//Return:	a4003_t_rowtype&
	a4003_t_rowtype & GetRow(long lRowNum)
	{
		if(lRowNum>=0&&lRowNum<(long)m_DATAcorpora.size())
		return m_DATAcorpora[lRowNum];
		else 
		{
			a4003_t_rowtype tmpEmptyrow;
...
//Return:	a4003_t_rowtype&
	a4003_t_rowtype & GetRow(const std::vector<long> & vRps, long lRowNum)
	{
		long ltmp;
		do{
		{
...
//Return:	no return
	void Add(const a4003_t_rowtype & varRow)
	{
...
//Return:	a4003_t_rowtype&
	a4003_t_rowtype & SelE1_strTblName(std::string strVal) 
	{
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4003_t_rowtype&
	a4003_t_rowtype & SelE1_lVer(long iVal) 
	{
		std::vector<long> vRps ;
		SelE_lVer( iVal, vRps );
		return GetRow( vRps, 0 );
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4003_t_rowtype&
	a4003_t_rowtype & SelE1_strAffectDateTime(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4003_t_rowtype&
	a4003_t_rowtype & SelE1_biDelFlag(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4003_t_rowtype&
	a4003_t_rowtype & SelE1_biIsAffect(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4003_t_rowtype&
	a4003_t_rowtype & SelE1_strStampDateTime(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4003_t_rowtype&
	a4003_t_rowtype & SelE1_strDataRef(std::string strVal) 
	{
		std::vector<long> vRps ;
...
/////////////////////////////////////////////////////////////////
//Function:	SelE_FareRate
//Effect:	select and build RPS where col=val use traverse
//Return:	no return
	void SelE_FareRate(ui16arr_t<64> aVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //(ni)
	{
		for(long ltmp=0;ltmp<(long)m_DATAcorpora.size();ltmp++)
		if(m_DATAcorpora[ltmp].m_FareRate==aVal)
		if( !pRefRps || !pRefRps->empty() && std::binary_search( pRefRps->begin(), pRefRps->end(), ltmp ) ) vRps.push_back( ltmp );
	}

/////////////////////////////////////////////////////////////////
//Function:	SelE1_FareRate
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4003_t_rowtype&
	a4003_t_rowtype & SelE1_FareRate(ui16arr_t<64> aVal) 
	{
		std::vector<long> vRps ;
		SelE_FareRate( aVal, vRps );
		return GetRow( vRps, 0 );
	}

/////////////////////////////////////////////////////////////
//Function:	SelEc_FareRate
//Effect:	select and build RPS where col=val. clear rps first.
//Return:	no return
	void SelEc_FareRate(ui16arr_t<64> aVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //()
	{
		vRps.clear();
		SelE_FareRate(aVal, vRps, pRefRps);
	}
...
//Effect:	select and build RPS where col=val use traverse
//Return:	no return
	void SelE_RES_01(u8arr_t<64> aVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //(ni)
	{
		for(long ltmp=0;ltmp<(long)m_DATAcorpora.size();ltmp++)
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4003_t_rowtype&
	a4003_t_rowtype & SelE1_RES_01(u8arr_t<64> aVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select and build RPS where col=val. clear rps first.
//Return:	no return
	void SelEc_RES_01(u8arr_t<64> aVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //()
	{
		vRps.clear();
...
////////////////////////////////////////////////////////////////
//Function:	SelE_lFareRateIdx
//Effect:	select and build RPS where col=val use traverse
//Return:	no return
	void SelE_lFareRateIdx(long iVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //(ni)
	{
		for(long ltmp=0;ltmp<(long)m_DATAcorpora.size();ltmp++)
		if(m_DATAcorpora[ltmp].m_lFareRateIdx==iVal)
		if( !pRefRps || !pRefRps->empty() && std::binary_search( pRefRps->begin(), pRefRps->end(), ltmp ) ) vRps.push_back( ltmp );
	}

///////////////////////////////////////////////////////
//Function:	SelE1_lFareRateIdx
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4003_t_rowtype&
	a4003_t_rowtype & SelE1_lFareRateIdx(long iVal) 
	{
		std::vector<long> vRps ;
		SelE_lFareRateIdx( iVal, vRps );
		return GetRow( vRps, 0 );
	}

///////////////////////////////////////////////////////////
//Function:	SelEc_lFareRateIdx
//Effect:	select and build RPS where col=val. clear rps first.
//Return:	no return
	void SelEc_lFareRateIdx(long iVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //()
	{
		vRps.clear();
		SelE_lFareRateIdx(iVal, vRps, pRefRps);
	}
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4003_t_rowtype&
	a4003_t_rowtype & SelE1_RES01(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4003_t_rowtype&
	a4003_t_rowtype & SelE1_RES02(u8arr_t<2> aVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4003_t_rowtype&
	a4003_t_rowtype & SelE1_RES03(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
