...
#ifndef V1_3AA4001_T_20140523_162548
#define V1_3AA4001_T_20140523_162548

 
///////////////////////////////////////////////////////
//Contents:	The row class type definition of a4001_t
//Name:	a4001_t_rowtype
class a4001_t_rowtype {
 
public:

	std::string		m_strTblName;			// 1		8.0.3  4001-节日表。此类交互用于向交互的接受方发送节日信息。交互的请求消息中 
	long		m_lVer;			// 2		参数版本 
	...
	std::string		m_strFestivalDate;			// a	1	 节日日期 YYYYMMDD，节日从该日期的0点开始，直到该日运营时间结束为止 
	wl::tuint8		m_RES01;			// a	2	[保留] 
	u8arr_t<2>		m_RES02;			// a	3	[保留] 
	wl::tuint8		m_RES03;			// a	4	[保留] 
...
public:
	...
	virtual ~a4001_t_rowtype(){;}
	

	//operator == 
	bool operator == (const a4001_t_rowtype & rhs) const
	{
		return this == &rhs;
	}
...
public:
	...
	std::string & Serialize( std::string & strOut )
	{
		...
		strOut += std::string(buf2);
		strOut += std::string("/");
		buf1 = (const char *)m_strFestivalDate.c_str();
		len1 = (long)(m_strFestivalDate.length()*sizeof(char)+sizeof(char));
		if( (int)v.size() < ( len1 * 2 + 4 ) ) v.resize( len1 * 2 + 4 );
		buf2 = (char*)(&(v[0]));
		en( buf1, len1, buf2 );
		strOut += std::string(buf2);
	...
	a4001_t_rowtype & Unserialize( const char * strIn )
	{
		const char *buf1;
		char *buf2;
		buf1 = strIn;
		...
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
		 /*if( *buf2!='}')*/ m_strFestivalDate = (char*)buf2; }else return *this; 
		buf2 = (char*)buf1;
		 if( *buf2!='}') {buf1 = de( buf2 );  
	...
	a4001_t_rowtype & Unserialize( wl::tchar * p_begin, wl::tchar ** pp_end )
	{
		char *buf1 = p_begin;
		char *buf2;
		buf2 = (char*)buf1 + 4;
...
////////////////////////////////////////////////////////////////
//Function:	GetCol_strFestivalDate
//Effect:	get ref of col. col is strFestivalDate
//Return:	std::string & 
	std::string & GetCol_strFestivalDate(void)
	{
		return m_strFestivalDate;
	}
...
private:
	...
	a4001_t_rowtype m_EmptyRow;
	std::vector<a4001_t_rowtype>  m_DATAcorpora;
 
public:

	typedef a4001_t_rowtype 	ROWTYPE;
	typedef std::vector<long> 	RPSTYPE; //RPS(return pointer set) type definition
	typedef std::vector<a4001_t_rowtype>::iterator 	TBLITTYPE;
...
public:
	...
	//operator == 
	bool operator == (const a4001_t & rhs) const
	{
		return this == &rhs;
	}
...
public:
	...
	a4001_t &  Unserialize( const char * strIn , int biAppend = 0 ) //if append==0, it would over-write the current tbl. 
	{
		char *p1,*p2;
		p1 = (char*)strIn;
		if(biAppend==0) Clear();
	...
	a4001_t & Unserialize( wl::SCake & ckIn , int biAppend = 0 )
	{
		char *p1,*p2;
		p1 = p2 = (char*)ckIn.buf();
		if(biAppend==0) Clear();
...
//Return:	a4001_t_rowtype&
	a4001_t_rowtype & GetRow(long lRowNum)
	{
		if(lRowNum>=0&&lRowNum<(long)m_DATAcorpora.size())
		return m_DATAcorpora[lRowNum];
		else 
		{
			a4001_t_rowtype tmpEmptyrow;
...
//Return:	a4001_t_rowtype&
	a4001_t_rowtype & GetRow(const std::vector<long> & vRps, long lRowNum)
	{
		long ltmp;
		do{
		{
...
//Return:	no return
	void Add(const a4001_t_rowtype & varRow)
	{
...
//Return:	a4001_t_rowtype&
	a4001_t_rowtype & SelE1_strTblName(std::string strVal) 
	{
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4001_t_rowtype&
	a4001_t_rowtype & SelE1_lVer(long iVal) 
	{
		std::vector<long> vRps ;
		SelE_lVer( iVal, vRps );
		return GetRow( vRps, 0 );
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4001_t_rowtype&
	a4001_t_rowtype & SelE1_strAffectDateTime(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4001_t_rowtype&
	a4001_t_rowtype & SelE1_biDelFlag(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4001_t_rowtype&
	a4001_t_rowtype & SelE1_biIsAffect(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4001_t_rowtype&
	a4001_t_rowtype & SelE1_strStampDateTime(std::string strVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4001_t_rowtype&
	a4001_t_rowtype & SelE1_strDataRef(std::string strVal) 
	{
		std::vector<long> vRps ;
...
///////////////////////////////////////////////////////////////
//Function:	SelE_strFestivalDate
//Effect:	select and build RPS where col=val use traverse
//Return:	no return
	void SelE_strFestivalDate(std::string strVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //(ni)
	{
		for(long ltmp=0;ltmp<(long)m_DATAcorpora.size();ltmp++)
		if(m_DATAcorpora[ltmp].m_strFestivalDate==strVal)
		if( !pRefRps || !pRefRps->empty() && std::binary_search( pRefRps->begin(), pRefRps->end(), ltmp ) ) vRps.push_back( ltmp );
	}

////////////////////////////////////////////////////////////////
//Function:	SelE1_strFestivalDate
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4001_t_rowtype&
	a4001_t_rowtype & SelE1_strFestivalDate(std::string strVal) 
	{
		std::vector<long> vRps ;
		SelE_strFestivalDate( strVal, vRps );
		return GetRow( vRps, 0 );
	}

/////////////////////////////////////////////////////////////////
//Function:	SelEc_strFestivalDate
//Effect:	select and build RPS where col=val. clear rps first.
//Return:	no return
	void SelEc_strFestivalDate(std::string strVal, std::vector<long> & vRps, std::vector<long> * pRefRps=NULL) //()
	{
		vRps.clear();
		SelE_strFestivalDate(strVal, vRps, pRefRps);
	}
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4001_t_rowtype&
	a4001_t_rowtype & SelE1_RES01(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4001_t_rowtype&
	a4001_t_rowtype & SelE1_RES02(u8arr_t<2> aVal) 
	{
		std::vector<long> vRps ;
...
//Effect:	select 1st row where col=val, or return default row.
//Return:	a4001_t_rowtype&
	a4001_t_rowtype & SelE1_RES03(wl::tuint8 iVal) 
	{
		std::vector<long> vRps ;
